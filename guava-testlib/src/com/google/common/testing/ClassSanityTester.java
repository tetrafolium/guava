 ...
 * case is to test static factory classes like: <pre> interface Book {...}
 * public class Books {
...
public final class ClassSanityTester {
  ...
  private final ListMultimap<Class<?>, Object> distinctValues =
   ...
   * construction requires a parameter of {@code type}, the distinct values of
   * {@code type} can be passed as parameters to create {@code Foo} instances
   * that are unequal.
   *
   * <p>Calling {@code setDistinctValues(type, v1, v2)} also sets the default
   * value for {@code type} that's used for {@link #testNulls}.
   *
   * <p>Only necessary for types where {@link ClassSanityTester} doesn't already
   * know how to create distinct values.
   *
   * @return this tester instance
   * @since 17.0
   */
  public <T> ClassSanityTester setDistinctValues(Class<T> type, T value1,
      T value2) {
    checkNotNull(type);
    checkNotNull(value1);
    checkNotNull(value2);
   ...
   * parameters that aren't annotated with {@link Nullable}. In details: <ul>
   * <li>All non-private static methods are checked such that passing null for
   * any parameter that's not annotated with {@link javax.annotation.Nullable}
   * should throw {@link NullPointerException}. <li>If there is any non-private
   * constructor or non-private static factory method declared by
   *     {@code cls}, all non-private instance methods will be checked too using
   ...
   *     {@code cls}:
   *     <ul>
   *     <li>Test will fail if default value for a parameter cannot be
   * determined. <li>Test will fail if the factory method returns null so
   * testing instance methods is impossible. <li>Test will fail if the
   * constructor or factory method throws exception.
   *     </ul>
   ...
   * <li>Nulls test is not performed on method return values unless the method
  ...
  private boolean hasInstanceMethodToTestNulls(Class<?> c,
      Visibility visibility) {
    for (Method method :
        nullPointerTester.getInstanceMethodsToTest(c, visibility)) {
   ...
   * In details: <ul> <li>The non-private constructor or non-private static
   * factory method with the most parameters is used to construct the sample
   * instances. In case of tie, the candidate constructors or factories are
   * tried one after another until one can be used to construct sample
   * instances. <li>For the constructor or static factory method used to
   * construct instances, it's checked that when equal parameters are passed,
   * the result instance should also be equal; and vice versa. <li>If a
   * non-private constructor or non-private static factory method exists: <ul>
   *     <li>Test will fail if default value for a parameter cannot be
   * determined. <li>Test will fail if the factory method returns null so
   * testing instance methods is impossible. <li>Test will fail if the
   * constructor or factory method throws exception.
   ...
   * <li>Equality test is not performed on method return values unless the
   * method is a non-private static factory method whose return type is {@code
   * cls} or {@code cls}'s subtype. <li>Inequality check is not performed
   * against state mutation methods such as {@link List#add}, or functional
   * update methods such as {@link com.google.common.base.Joiner#skipNulls}.
   * </ul>
   *
   * <p>Note that constructors taking a builder object cannot be tested
   * effectively because semantics of builder can be arbitrarily complex. Still,
   * a factory class can be created in the test to facilitate equality testing.
   * For example: <pre> public class FooTest {
   *
   ...
   * }
   * </pre>
   * <p>It will test that Foo objects created by the {@code create(a, b, c, d)}
  ...
  ParameterHasNoDistinctValueException, IllegalAccessException,
  InvocationTargetException, FactoryMethodReturnsNullException {
    if (cls.isEnum()) {
      return;
    }
    List<? extends Invokable<?, ?>> factories =
        Lists.reverse(getFactories(TypeToken.of(cls)));
    ...
    List<ParameterHasNoDistinctValueException> distinctValueErrors =
        Lists.newArrayList();
    List<InvocationTargetException> instantiationExceptions =
        Lists.newArrayList();
   ...
   * non-private static factory methods with the parameters automatically
   * provided using dummy values.
   *
  ...
  /**
   * Returns an object responsible for performing sanity tests against the
   * return values of all public static methods declared by {@code cls},
  ...
  /**
   * Runs sanity tests against return values of static factory methods declared
   * by a class.
   */
  public final class FactoryMethodReturnValueTester {
    ...
    /**
     * Specifies that only the methods that are declared to return {@code
     * returnType} or its subtype are tested.
     *
    ...
    /**
     * Tests null checks against the instance methods of the return values, if
     * any.
     *
     * <p>Test fails if default value cannot be determined for a constructor or
     * factory method parameter, or if the constructor or factory method throws
     * exception.
     *
     * @return this tester
     */
    public FactoryMethodReturnValueTester testNulls() throws Exception {
      for (Invokable<?, ?> factory : getFactoriesToTest()) {
        ...
        if (instance != null &&
            packagesToTest.contains(
              Reflection.getPackageName(instance.getClass()))) {
          try {
    ...
    /**
     * Tests {@link Object#equals} and {@link Object#hashCode} against the
     * return values of the static methods, by asserting that when equal
     * parameters are passed to the same static method, the return value should
     * also be equal; and vice versa.
     *
     * <p>Test fails if default value cannot be determined for a constructor or
     * factory method parameter, or if the constructor or factory method throws
     * exception.
     ...
     * @return this tester
     */
    public FactoryMethodReturnValueTester testEqualsAndSerializable()
    throws Exception {
      for (Invokable<?, ?> factory : getFactoriesToTest()) {
        try {
          testEqualsUsing(factory);
        ...
        if (instance != null) {
          try {
            ...
            AssertionError error =
                new AssertionFailedError("Return value of " + factory +
                " reserialized to an unequal value");
            error.initCause(e);
            throw error;
          }
        }
    ...
    private ImmutableList<Invokable<?, ?>> getFactoriesToTest() {
      ImmutableList.Builder<Invokable<?, ?>> builder = ImmutableList.builder();
      for (Invokable<?, ?> factory : factories) {
        if (returnTypeToTest.isAssignableFrom(
              factory.getReturnType().getRawType())) {
          builder.add(factory);
        }
      }
      ...
      Assert.assertFalse("No " + factoryMethodsDescription + " that return " +
          ...
          " or subtype are found in " + declaringClass + ".",
  ...
  /**
   * Instantiates using {@code factory}. If {@code factory} is annotated with
   * {@link Nullable} and returns null, null will be returned.
   *
   * @throws ParameterNotInstantiableException if the static methods cannot be
   *     invoked because
   *         the default value of a parameter cannot be determined.
   * @throws IllegalAccessException if the class isn't public or is nested
   *     inside a non-public
  ...
  InvocationTargetException, FactoryMethodReturnsNullException {
    ...
    List<FreshValueGenerator> argGenerators =
        Lists.newArrayListWithCapacity(params.size());
    ...
    List<Object> equalArgs =
        generateEqualFactoryArguments(factory, params, args);
    ...
    EqualsTester tester = new EqualsTester(new ItemReporter() {
      ...
      String reportItem(Item<?> item) {
        List<Object> factoryArgs =
        argGroups.get(item.groupNumber).get(item.itemNumber);
    ...
    for (int i = 0; i < params.size(); i++) {
      ...
      Object newArg =
          argGenerators.get(i).generateFresh(params.get(i).getType());
   ...
   * equality group.
   */
  private List<Object> generateEqualFactoryArguments(Invokable<?, ?> factory,
      List<Parameter> params,
      List<Object> args)
  throws ParameterNotInstantiableException,
  FactoryMethodReturnsNullException, InvocationTargetException,
  IllegalAccessException {
    List<Object> equalArgs = Lists.newArrayList(args);
    for (int i = 0; i < args.size(); i++) {
      Parameter param = params.get(i);
      Object arg = args.get(i);
      // Use new fresh value generator because 'args' were populated with new
      // fresh generator each. Two newFreshValueGenerator() instances should
      // normally generate equal value sequence.
      ...
      if (arg != shouldBeEqualArg && Objects.equal(arg, shouldBeEqualArg) &&
          ...
          hashCodeInsensitiveToArgReference(
            factory, args, i,
            generateDummyArg(param, newFreshValueGenerator()))) {
        // If the implementation uses identityHashCode(), referential equality
        // is probably intended. So no point in using an equal-but-different
        // factory argument. We check twice to avoid confusion caused by
        // accidental hash collision.
  ...
  hashCodeInsensitiveToArgReference(Invokable<?, ?> factory, List<Object> args,
      int i, Object alternateArg)
  ...
  // type-safe data structure to hold the mappings.
    ...
    FreshValueGenerator generator = new FreshValueGenerator() {
      ...
      Object interfaceMethodCalled(Class<?> interfaceType, Method method) {
        return getDummyValue(
          TypeToken.of(interfaceType).method(method).getReturnType());
      }
    };
    for (Map.Entry<Class<?>, Collection<Object>> entry :
        distinctValues.asMap().entrySet()) {
  ...
  @Nullable
  private static Object generateDummyArg(Parameter param,
  ...
  private static <X extends Throwable> void throwFirst(List<X> exceptions)
  throws X {
  ...
  getFactories(TypeToken<T> type) {
    ...
    for (Method method : type.getRawType().getDeclaredMethods()) {
      ...
      if (!invokable.isPrivate() && !invokable.isSynthetic() &&
        ...
        Invokable<?, ? extends T> factory =
            (Invokable<?, ? extends T>)invokable;
        factories.add(factory);
      }
    }
    if (!Modifier.isAbstract(type.getRawType().getModifiers())) {
      for (Constructor<?> constructor :
          type.getRawType().getDeclaredConstructors()) {
    ...
    // Sorts methods/constructors with least number of parameters first since
    // it's likely easier to fill dummy parameter values for them. Ties are
    // broken by name then by the string form of the parameter list.
    return BY_NUMBER_OF_PARAMETERS.compound(BY_METHOD_NAME)
  ...
  private <T> T getDummyValue(TypeToken<T> type) {
    ...
    // Assume all default values are generics safe.
    T defaultValue = (T)defaultValues.getInstance(rawType);
    if (defaultValue != null) {
      return defaultValue;
    }
    @SuppressWarnings("unchecked")
    // ArbitraryInstances always returns generics-safe dummies.
  ...
  private static <T> T createInstance(Invokable<?, ? extends T> factory,
      List<?> args)
  throws FactoryMethodReturnsNullException, InvocationTargetException,
  IllegalAccessException {
  ...
  /**
   * Thrown if the test tries to invoke a constructor or static factory method
   * but failed because the dummy value of a constructor or method parameter is
   * unknown.
   */
  @VisibleForTesting
  static class ParameterNotInstantiableException extends Exception {
    public ParameterNotInstantiableException(Parameter parameter) {
      super("Cannot determine value for parameter " + parameter + " of " +
  ...
  static class FactoryMethodReturnsNullException extends Exception {
    public FactoryMethodReturnsNullException(Invokable<?, ?> factory) {
      super(factory +
      " returns null and cannot be used to test instance methods.");
    }
  }

  private static final class SerializableDummyProxy
    extends DummyProxy implements Serializable {

    private final transient ClassSanityTester tester;

    SerializableDummyProxy(ClassSanityTester tester) { this.tester = tester; }
