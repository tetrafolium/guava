 ...
 *
 * <p><b>Warning:</b> as with any sorted collection, you are strongly advised
 * not to use a {@link Comparator} or {@link Comparable} type whose comparison
...
public abstract class ImmutableSortedMultiset<E>
   ...
   */
  public static <E extends Comparable<? super E>> ImmutableSortedMultiset<E>
  of(E element) {
  ...
  /**
   * Returns an immutable sorted multiset containing the given elements sorted
  ...
  /**
   * Returns an immutable sorted multiset containing the given elements sorted
   * by their natural ordering. To create a copy of a {@code SortedMultiset}
   * that preserves the comparator, call {@link #copyOfSorted} instead. This
   * method iterates over {@code elements} at most once.
   *
   * <p>Note that if {@code s} is a {@code Multiset<String>}, then {@code
   * ImmutableSortedMultiset.copyOf(s)} returns an {@code
   * ImmutableSortedMultiset<String>} containing each of the strings in {@code
   * s}, while {@code ImmutableSortedMultiset.of(s)} returns an {@code
   * ImmutableSortedMultiset<Multiset<String>>} containing one element (the
  ...
  /**
   * Returns an immutable sorted multiset containing the given elements sorted
   * by the given {@code Comparator}. This method iterates over {@code elements}
   * at most once.
   *
   * <p>Despite the method name, this method attempts to avoid actually copying
   * the data when it is safe to do so. The exact circumstances under which a
  ...
  copyOf(Comparator<? super E> comparator, Iterable<? extends E> elements) {
    if (elements instanceof ImmutableSortedMultiset) {
      ...
      ImmutableSortedMultiset<E> multiset =
          (ImmutableSortedMultiset<E>)elements;
   ...
   * copy will or will not be performed are undocumented and subject to change.
   *
   * <p>This method is safe to use even when {@code sortedMultiset} is a
   * synchronized or concurrent collection that is currently being modified by
   * another thread.
   *
   * @throws NullPointerException if {@code sortedMultiset} or any of its
   *     elements is null
   */
  public static <E> ImmutableSortedMultiset<E>
  ...
  copyOfSortedEntries(Comparator<? super E> comparator,
    ...
    ImmutableList.Builder<E> elementsBuilder =
        new ImmutableList.Builder<E>(entries.size());
  ...
  @Override public abstract ImmutableSortedSet<E> elementSet();
   ...
   * {@inheritDoc}
   *
   * <p>This implementation is guaranteed to throw an {@link
   ...
   * generated, such as creating a {@code SortedMultiset<Integer>} with a {@code
  ...
  /**
   * Returns a builder that creates immutable sorted multisets whose elements
   * are ordered by the reverse of their natural ordering.
   *
   * <p>Note: the type parameter {@code E} extends {@code Comparable<?>} rather
  ...
  /**
   * Returns a builder that creates immutable sorted multisets whose elements
   * are ordered by their natural ordering. The sorted multisets use {@link
  ...
  public static class Builder<E> extends ImmutableMultiset.Builder<E> {
    /*
     * We keep an array of elements and counts.  Periodically -- when we need
     * more room in the array, or when we're building, or the like -- we sort,
     * deduplicate, and combine the counts. Negative counts indicate a setCount
     * operation with ~counts[i].
     */
    ...
    /*
     * The number of used positions in the elements array.  We deduplicate
     * periodically, so this may fluctuate up and down.
     */
    private int length;

    // True if we just called build() and the elements array is being used by a
    // created ISM, meaning we shouldn't modify that array further.
    ...
    private void maintenance() {
      if (length == elements.length) {
        ...
        // we don't currently need to copy the counts array, because we don't
        // use it directly in built ISMs
      }
    ...
    private void dedupAndCoalesce(boolean maybeExpand) {
      ...
      for (int i = 1; i < sortedElements.length; i++) {
        if (comparator.compare(sortedElements[uniques - 1], sortedElements[i]) <
            0) {
      ...
      // Note that we're not getting rid, yet, of elements with count 0.  We'll
      // do that in build().
     ...
     *     if this operation
     *         would result in more than {@link Integer#MAX_VALUE} occurrences
     * of the element
     */
    @CanIgnoreReturnValue
    @Override
    ...
    /**
     * Adds each element of {@code elements} to the {@code
     * ImmutableSortedMultiset}.
     *
     ...
     * ImmutableSortedMultiset}.
     *
     * @param elements the elements to add to the {@code
     *     ImmutableSortedMultiset}
     * @return this {@code Builder} object
     * @throws NullPointerException if {@code elements} is null or contains a
     *     null element
    ...
    private void dedupAndCoalesceAndDeleteEmpty() {
      ...
      // If there was a setCount(elem, 0), those elements are still present.
      // Eliminate them.
      int size = 0;
      for (int i = 0; i < length; i++) {
        if (counts[i] > 0) {
    ...
    /**
     * Returns a newly-created {@code ImmutableSortedMultiset} based on the
