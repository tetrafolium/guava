...
/**
 * Basic implementation of a primitive map of Object keys to counting number
 * values, to be used for backing store for {@link Multiset} implementations.
 */
...
abstract class AbstractObjectCountMap<K> {
   ...
   * @return the previous value associated with key, or 0 if there was no
   *     mapping for key.
   */
  @CanIgnoreReturnValue abstract int put(@Nullable K key, int value);
   ...
   * @return the int value to which the specified key is mapped, or 0 if this
   ...
   *
   * <p>Returns the value to which this map previously associated the key, or 0
   * if the map contained no mapping for the key.
   *
   * @param key key whose mapping is to be removed from the map
   * @return the previous int value associated with key, or 0 if there was no
   *     mapping for key.
   */
  @CanIgnoreReturnValue abstract int remove(@Nullable Object key);
  ...
  /**
   * Returns index of the specified key in the underlying key array.
   * Implementation specific and does not denote ordering of the keys.
   */
  abstract int indexOf(@Nullable Object key);

  /** Given the index of a key, remove the corresponding entry in the map. */
  @CanIgnoreReturnValue abstract int removeEntry(int entryIndex);

  Set<K> createKeySet() { return new KeySetView(); }
  ...
  int nextIndex(int index) { return (index + 1 < size) ? index + 1 : -1; }
  ...
  /**
   * Returns a Set view of the entries contained in this map. The set is backed
  ...
  Set<Entry<K>> entrySet() {
    return (entrySetView == null) ? entrySetView = createEntrySet()
                                  : entrySetView;
  }
