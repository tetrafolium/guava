 ...
 * }</pre>
 *
 * <p>can be iterated over in preorder (hdabcegf), postorder (abcdefgh), or
 * breadth-first order (hdegabcf).
 *
 * <p>Null nodes are strictly forbidden.
 *
 * <p><b>For Java 8 users:</b> Because this is an abstract class, not an
 * interface, you can't use a lambda expression to extend it:
 *
 * <pre>{@code
 * // won't work
 ...
 *
 * <pre>{@code
 * TreeTraverser<NodeType> traverser = TreeTraverser.using(node ->
 * node.getChildNodes());
 * }</pre>
 *
...
public abstract class TreeTraverser<T> {
   ...
   *
   * <p>No guarantees are made about the behavior of the traversal when nodes
  ...
  /**
   * Returns an unmodifiable iterable over the nodes in a tree structure, using
   * post-order traversal. That is, each node's subtrees are traversed before
  ...
  /**
   * Returns an unmodifiable iterable over the nodes in a tree structure, using
   * breadth-first traversal. That is, all the nodes of depth 0 are returned,
