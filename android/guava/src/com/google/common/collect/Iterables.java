 ...
 * {@code Iterable}. Except as noted, each method has a corresponding {@link
 * Iterator}-based method in the {@link Iterators} class.
 *
 * <p><b>Java 8 users:</b> several common uses for this class are now more
...
public final class Iterables {
  ...
  /**
   * Returns {@code true} if {@code iterable} contains any element {@code o} for
   * which {@code Objects.equals(o, element)} would return {@code true}.
   * Otherwise returns {@code false}, even in cases where {@link
   * Collection#contains} might throw {@link NullPointerException} or {@link
   * ClassCastException}.
   */
  ...
  removeIfFromRandomAccessList(List<T> list, Predicate<? super T> predicate) {
    // Note: Not all random access lists support set(). Additionally, it's
    // possible for a list to reject setting an element, such as when the list
    // does not permit duplicate elements. For both of those cases,  we need to
  ...
  slowRemoveIfForRemainingElements(List<T> list, Predicate<? super T> predicate,
                                   int to, int from) {
    // Here we know that:
   ...
   *
   * <p><b>Java 8 users:</b> the {@code Stream} equivalent to this method is
   * {@code stream.collect(MoreCollectors.onlyElement())}.
   *
   ...
   *
   * <p><b>Java 8 users:</b> the {@code Stream} equivalent to this method is
   * {@code stream.collect(MoreCollectors.toOptional()).orElse(defaultValue)}.
   *
   ...
   *
   * <p><b>Java 8 users:</b> In most cases, the {@code Stream} equivalent of
   * this method is {@code stream.filter(element::equals).count()}. If {@code
   * element} might be null, use {@code
   ...
   * element, which is no longer in {@code iterable}. The iterator's {@code
   * hasNext()} method returns {@code true} until {@code iterable} is empty.
   *
   ...
   *
   * <p><b>Java 8 users:</b> The {@code Stream} equivalent of this method is
   * {@code Stream.generate(() -> iterable).flatMap(Streams::stream)}.
   */
   ...
   *
   * <p><b>Java 8 users:</b> If passing a single element {@code e}, the {@code
   * Stream} equivalent of this method is {@code Stream.generate(() -> e)}.
   * Otherwise, put the elements in a collection and use {@code
   ...
   * has an iterator that traverses the elements of each iterable in {@code
   ...
   *
   * <p><b>Java 8 users:</b> The {@code Stream} equivalent of this method is
   * {@code streamOfStreams.flatMap(s -> s)}.
   */
  public static <T> Iterable<T>
  ...
  paddedPartition(final Iterable<T> iterable, final int size) {
    checkNotNull(iterable);
    checkArgument(size > 0);
    return new FluentIterable<List<T>>() {
      @Override
   ...
   * the input predicate
   * {@code retainIfTrue}. The returned iterable's iterator does not support
   * {@code remove()}.
   *
   ...
   *
   * <p><b>{@code Stream} equivalent:</b> {@code
   * stream.filter(predicate).findFirst().get()}
   *
   ...
   *
   * <p><b>{@code Stream} equivalent:</b> {@code
   * stream.skip(position).findFirst().get()} (throws {@code
   ...
   *
   * <p>To get the only element in a single-element {@code Iterable}, consider
   * using {@link #getOnlyElement(Iterable)} or {@link #getOnlyElement(Iterable,
   * Object)} instead.
   *
   * <p><b>{@code Stream} equivalent:</b> {@code
   ...
   * {@code O(1)}.
   *
   * <p><b>{@code Stream} equivalent:</b> {@link Streams#findLast
   ...
   *
   * <p><b>{@code Stream} equivalent:</b> {@code
   * Streams.findLast(stream).orElse(defaultValue)}
   *
