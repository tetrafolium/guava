 ...
 * <a href="https://github.com/google/guava/wiki/EventBusExplained">{@code
 * EventBus}</a>.
 *
 ...
 * CustomerChangeEvent as its sole argument, and mark it with the {@link
 * com.google.common.eventbus.Subscribe} annotation.
 * </ul>
 *
 ...
 * producer's
 *     {@code registerCustomerChangeEventListener} method. These methods are
 * rarely defined in common interfaces, so in addition to knowing every possible
 * producer, you must also know its type. <li><strong>...with EventBus:</strong>
 * pass your object to the
 ...
 * event-dispatching method (perhaps using AOP). <li><strong>...with
 * EventBus:</strong> subscribe to {@link com.google.common.eventbus.DeadEvent}.
 ...
 * class like EventListenerList. <li><strong>...with EventBus:</strong> EventBus
 * does this for you.
 * </ul>
 *
 ...
 * desired) asynchronicity. <li><strong>...with EventBus:</strong> pass the
 * event object to an EventBus's
 ...
 * events. Subscriber methods are marked by the {@link
 * com.google.common.eventbus.Subscribe} annotation.
 *
 * <dt>Posting an event
 ...
 *
 * <p>The Event Bus doesn't specify how you use it; there's nothing stopping
 * your application from having separate EventBus instances for each component,
 * or using separate instances to separate events by context or topic. This also
 ...
 * disadvantages: <ul> <li>Any one class can only implement a single response to
 * a given event. <li>Listener interface methods may conflict. <li>The method
 * must be named after the event (e.g. {@code handleChangeEvent}), rather than
 * its purpose (e.g. {@code recordChangeInJournal}). <li>Each event usually has
 * its own interface, without a common parent interface for a family of events
 * (e.g. all UI events).
 * </ul>
 *
 ...
 *   }}</pre>
 *
 * <p>The intent is actually clearer in the second case: there's less noise
 * code, and the event subscriber has a clear and meaningful name.
 *
 ...
 *   }}</pre>
 *
 * <p>Due to erasure, no single class can implement a generic interface more
 * than once with different type parameters. This is a giant step backwards from
 ...
 *
 * <p>This way, any object created by the container/factory/environment can hook
 ...
 * methods. Specifically, any methods marked with
 ...
 * indicate an error (typically an indication that you missed a {@code
 ...
 *
 * <p>To handle such events, register a subscriber method for the {@code
 * DeadEvent} class. Whenever EventBus receives an event with no registered
 * subscribers, it will turn it into a
 * {@code DeadEvent} and pass it your way -- allowing you to log it or otherwise
 * recover.
 *
 * <h3>How do I test event listeners and their subscriber methods?</h3>
 *
 * <p>Because subscriber methods on your listener classes are normal methods,
 * you can simply call them from your test code to simulate the EventBus.
 */
@CheckReturnValue
