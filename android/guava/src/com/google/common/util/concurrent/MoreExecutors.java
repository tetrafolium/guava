...
public final class MoreExecutors {
  ...
  /**
   * Add a shutdown hook to wait for thread completion in the given {@link
   * ExecutorService service}. This is useful if the given service uses daemon
   * threads, and we want to keep the JVM from exiting immediately on shutdown,
   * instead giving these daemon threads a chance to terminate normally.
   *
  ...
  private static final class DirectExecutorService
    ...
    public boolean isTerminated() {
      synchronized (lock) { return shutdown && runningTasks == 0; }
    }
      ...
      synchronized (lock) {
        while (true) {
          if (shutdown && runningTasks == 0) {
            ...
            nanos -=
                System.nanoTime() - now; // subtract the actual time we waited
          }
        }
      }
    }
   ...
   * {@code invokeAll} or
   * {@code invokeAny}. In the latter case, tasks will run serially on the
   ...
   *
   * <p>Although all tasks are immediately executed in the thread that submitted
   * the task, this
   * {@code ExecutorService} imposes a small locking overhead on each task
   ...
   * to tasks given to {@code invokeAll} or
   * {@code invokeAny} which are pending serial execution, even the subset of
   * the tasks that have not yet started execution. It is unclear from the
   * {@code ExecutorService} specification if these should be included, and it's
   * much easier to implement the interpretation that they not be. Finally, a
   * call to {@code shutdown} or {@code shutdownNow} may result in concurrent
   * calls to {@code invokeAll/invokeAny} throwing RejectedExecutionException,
   * although a subset of the tasks may already have been executed.
   *
   ...
   *   }}</pre>
   *
   * <p>This should be preferred to {@link #newDirectExecutorService()} because
   * implementing the
   * {@link ExecutorService} subinterface necessitates significant performance
   * overhead.
   *
   * @since 18.0
   */
  ...
  /**
   * Returns an {@link Executor} that runs each task executed sequentially, such
   * that no two tasks are running concurrently.
   *
   * <p>The executor uses {@code delegate} in order to {@link Executor#execute
   * execute} each task in turn, and does not create any threads of its own.
   *
   * <p>After execution starts on the {@code delegate} {@link Executor}, tasks
   * are polled and executed from the queue until there are no more tasks. The
   * thread will not be released until there are no more tasks to run.
   *
   * <p>If a task is {@linkplain Thread#interrupt interrupted}, execution of
   * subsequent tasks continues. {@code RuntimeException}s thrown by tasks are
   * simply logged and the executor keeps trucking. If an {@code Error} is
   * thrown, the error will propagate and execution will stop until the next
   ...
   * never calls the delegate's {@code submit},
   ...
   */
  @GwtIncompatible // TODO
  public static ListeningExecutorService
   ...
   */
  @GwtIncompatible // TODO
  public static ListeningScheduledExecutorService
   ...
   *
   * Other contributors include Andrew Wright, Jeffrey Hayes, Pat Fisher, Mike
   * Judd.
   */

  /**
   * An implementation of {@link ExecutorService#invokeAny} for {@link
   * ListeningExecutorService} implementations.
   */
  @GwtIncompatible
   ...
   *
   * <p>On AppEngine, returns {@code
   * ThreadManager.currentRequestThreadFactory()}. Otherwise, returns {@link
  ...
  private static boolean isAppEngine() {
    ...
    try {
      ...
      // If the method doesn't exist, we're not on a supported version of
      // AppEngine;
      return false;
    }
  }
  ...
  // ListeningScheduledExecutorService?
  // TODO(lukes): provide overloads that take constant strings?
  // Function<Runnable, String>s to calculate names?

  /**
   * Creates an {@link Executor} that renames the {@link Thread threads} that
   * its tasks run in.
   *
   * <p>The names are retrieved from the {@code nameSupplier} on the thread that
   * is being renamed right before each task is run. The renaming is best
   * effort, if a {@link SecurityManager} prevents the renaming then it will be
   * skipped but the tasks will still execute.
   *
   *
   ...
   * submissions and later, if necessary, cancelling remaining tasks.
   *
   * <p>The method takes the following steps:
   * <ol>
   * <li>calls {@link ExecutorService#shutdown()}, disabling acceptance of new
   * submitted tasks. <li>awaits executor service termination for half of the
   * specified timeout. <li>if the timeout expires, it calls {@link
   * ExecutorService#shutdownNow()}, cancelling pending tasks and interrupting
   * running tasks. <li>awaits executor service termination for the other half
   * of the specified timeout.
   * </ol>
   *
   * <p>If, at any step of the process, the calling thread is interrupted, the
   * method calls
   ...
   *     successfully, {@code false} if the call timed out or was interrupted
   * @since 17.0
   */
  @Beta
