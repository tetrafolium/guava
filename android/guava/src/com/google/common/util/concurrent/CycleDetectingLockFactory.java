 ...
 * and
 * {@link ReentrantReadWriteLock} instances that detect potential deadlock by
 * checking for cycles in lock acquisition order.
 *
 * <p>Potential deadlocks detected when calling the {@code lock()}, {@code
 * lockInterruptibly()}, or
 * {@code tryLock()} methods will result in the execution of the {@link Policy}
 ...
 * </ul>
 *
 * <p>The locks created by a factory instance will detect lock acquisition
 * cycles with locks created by other {@code CycleDetectingLockFactory}
 * instances (except those with {@code Policy.DISABLED}). A lock's behavior when
 * a cycle is detected, however, is defined by the {@code Policy} of the factory
 * that created it. This allows detection of cycles across components while
 * delegating control over lock behavior to individual components.
 *
 * <p>Applications are encouraged to use a {@code CycleDetectingLockFactory} to
 ...
 *
 * <p>Deadlocks can arise when locks are acquired in an order that forms a
 * cycle. In a simple example involving two locks and two threads, deadlock
 * occurs when one thread acquires Lock A, and then Lock B, while another thread
 ...
 * </pre>
 *
 * <p>Neither thread will progress because each is waiting for the other. In
 * more complex applications, cycles can arise from interactions among more than
 * 2 locks:
 *
 * <pre>
 * Thread1: acquire(LockA) --X acquire(LockB)
 ...
 * </pre>
 *
 * <p>The implementation detects cycles by constructing a directed graph in
 * which each lock represents a node and each edge represents an acquisition
 * ordering between two locks. <ul> <li>Each lock adds (and removes) itself
 * to/from a ThreadLocal Set of acquired locks when the Thread acquires its
 * first hold (and releases its last remaining hold). <li>Before the lock is
 * acquired, the lock is checked against the current set of acquired locks---to
 * each of the acquired locks, an edge from the soon-to-be-acquired lock is
 * either verified or created. <li>If a new edge needs to be created, the
 * outgoing edges of the acquired locks are traversed to check for a cycle that
 * reaches the lock to be acquired. If no cycle is detected, a new "safe" edge
 * is created. <li>If a cycle is detected, an "unsafe" (cyclic) edge is created
 * to represent a potential deadlock situation, and the appropriate Policy is
 * executed.
 * </ul>
 *
 * <p>Note that detection of potential deadlock does not necessarily indicate
 * that deadlock will happen, as it is possible that higher level application
 * logic prevents the cyclic lock acquisition from occurring. One example of a
 * false positive is:
 *
 * <pre>
 * LockA -&gt; LockB -&gt; LockC
 ...
 *
 * <p>While {@code ReadWriteLock} instances have different properties and can
 * form cycles without potential deadlock, this class treats {@code
 * ReadWriteLock} instances as equivalent to traditional exclusive locks.
 * Although this increases the false positives that the locks detect (i.e.
 * cycles that will not actually result in deadlock), it simplifies the
 * algorithm and implementation considerably. The assumption is that a user of
 * this factory wishes to eliminate any cyclic acquisition ordering.
 *
 * <p><strong>Explicit Lock Acquisition Ordering</strong>
 *
 * <p>The {@link CycleDetectingLockFactory.WithExplicitOrdering} class can be
 * used to enforce an application-specific ordering in addition to performing
 * general cycle detection.
 *
 * <p><strong>Garbage Collection</strong>
 *
 * <p>In order to allow proper garbage collection of unused locks, the edges of
 * the lock graph are weak references.
 *
 * <p><strong>Performance</strong>
 *
 * <p>The extra bookkeeping done by cycle detecting locks comes at some cost to
 * performance. Benchmarks (as of December 2011) show that:
 *
 * <ul>
 * <li>for an unnested {@code lock()} and {@code unlock()}, a cycle detecting
 * lock takes 38ns as opposed to the 24ns taken by a plain lock. <li>for nested
 * locking, the cost increases with the depth of the nesting: <ul> <li>2 levels:
 * average of 64ns per lock()/unlock() <li>3 levels: average of 77ns per
 * lock()/unlock() <li>4 levels: average of 99ns per lock()/unlock() <li>5
 * levels: average of 103ns per lock()/unlock() <li>10 levels: average of 184ns
 * per lock()/unlock() <li>20 levels: average of 393ns per lock()/unlock()
 *     </ul>
 * </ul>
 *
...
public class CycleDetectingLockFactory {
  ...
  public interface Policy {
     ...
     *
     * <p>Note that the method will be called even upon an invocation of {@code
     * tryLock()}. Although
     * {@code tryLock()} technically recovers from deadlock by eventually timing
     * out, this behavior is chosen based on the assumption that it is the
     * application's wish to prohibit any cyclical lock acquisitions.
     */
  ...
  public enum Policies implements Policy {
     ...
     * of the
     * {@code PotentialDeadlockException} indicating the potential deadlock,
     ...
     * enabled.
     *
     * <p>Note that locks created by a factory with this policy will
     * <em>not</em> participate the cycle detection performed by locks created
     * by other factories.
     */
    DISABLED {
      @Override
  ...
  /**
   * Creates a {@link ReentrantLock} with the given fairness policy. The {@code
   * lockName} is used in the warning or exception output to help identify the
  ...
  public ReentrantReadWriteLock newReentrantReadWriteLock(String lockName,
    ...
    return policy == Policies.DISABLED
        ...
        : new CycleDetectingReentrantReadWriteLock(new LockGraphNode(lockName),
                                                   fair);
  }

  // A static mapping from an Enum type to its set of LockGraphNodes.
  private static final
      ConcurrentMap<Class<? extends Enum>, Map<? extends Enum, LockGraphNode>>
  ...
  private static Map<? extends Enum, LockGraphNode>
  getOrCreateNodes(Class<? extends Enum> clazz) {
   ...
   * and
   * {@code disallowedPriorLocks} prepopulated with nodes according to the
  ...
  static <E extends Enum<E>> Map<E, LockGraphNode> createNodes(Class<E> clazz) {
    ...
    for (int i = 0; i < numKeys - 1; i++) {
      nodes.get(i).checkAcquiredLocks(Policies.DISABLED,
  ...
  /**
   * For the given Enum value {@code rank}, returns the value's {@code
   * "EnumClass.name"}, which is used in exception and warning output.
   */
   ...
   * acquisitions. The application defines the allowed ordering with an {@code
   * Enum} whose values each correspond to a lock type. The order in which the
   * values are declared dictates the allowed order of lock acquisition. In
   * other words, locks corresponding to smaller values of {@link
   * Enum#ordinal()} should only be acquired before locks with larger ordinals.
   * Example:
   *
   * <pre>   {@code
   * enum MyLockOrder {
   ...
   *
   * <p>As with all locks created by instances of {@code
   * CycleDetectingLockFactory} explicitly ordered locks participate in general
   * cycle detection with all other cycle detecting locks, and a lock's behavior
   * when detecting a cyclic lock acquisition is defined by the {@code Policy}
   * of the factory that created it.
   *
   * <p>Note, however, that although multiple locks can be created for a given
  ...
  public static final class WithExplicitOrdering<E extends Enum<E>>
     ...
     * The values returned by {@link Enum#getDeclaringClass()} and {@link
     * Enum#name()} are used to describe the lock in warning or exception
     * output.
     *
     * @throws IllegalStateException If the factory has already created a {@code
     *     Lock} with the specified rank.
     */
    ...
    public ReentrantReadWriteLock newReentrantReadWriteLock(E rank,
      ...
      return policy == Policies.DISABLED
          ? new ReentrantReadWriteLock(fair)
          : new CycleDetectingReentrantReadWriteLock(lockGraphNodes.get(rank),
                                                     fair);
    }
  }
   ...
   * calls to
   * {@link #aboutToAcquire(CycleDetectingLock)} and {@link
   ...
   *   at
   * com...CycleDetectingReentrantLock.lock(CycleDetectingLockFactory.java:443)
   *   at ...
   *   at ...
   ...
   *
   * <pre>
   * com....PotentialDeadlockException: Potential Deadlock from LockC -&gt;
   * ReadWriteA at ... at ... Caused by: com...ExampleStackTrace: LockB -&gt;
   * LockC at ... at ... Caused by: com...ExampleStackTrace: ReadWriteA -&gt;
   * LockB at ... at ...
   * </pre>
   *
  ...
  public static final class PotentialDeadlockException
    ...
    private PotentialDeadlockException(
        LockGraphNode node1, LockGraphNode node2,
  ...
  private static class LockGraphNode {
    ...
    String getLockName() { return lockName; }
     ...
     *
     * <p>When this method returns, the {@code acquiredLock} should be in either
     * the
     * {@code preAcquireLocks} map, for the case in which it is safe to acquire
     * {@code this} after the {@code acquiredLock}, or in the {@code
     * disallowedPriorLocks} map, in which case it is not safe.
     */
    void checkAcquiredLock(Policy policy, LockGraphNode acquiredLock) {
      ...
      if (path == null) {
        ...
        allowedPriorLocks.put(acquiredLock,
                              new ExampleStackTrace(acquiredLock, this));
      } else {
     ...
     */
    @Nullable
    private ExampleStackTrace findPathTo(LockGraphNode node,
                                         Set<LockGraphNode> seen) {
      if (!seen.add(this)) {
  ...
  /**
   * CycleDetectingLock implementations must call this method in a {@code
   * finally} clause after any attempt to change the lock state, including both
   * lock and unlock attempts. Failure to do so can result in corrupting the
   * acquireLocks set.
   */
  ...
  final class CycleDetectingReentrantLock
    ...
    private CycleDetectingReentrantLock(LockGraphNode lockGraphNode,
                                        boolean fair) {
      super(fair);
  ...
  final class CycleDetectingReentrantReadWriteLock
    ...
    private CycleDetectingReentrantReadWriteLock(LockGraphNode lockGraphNode,
                                                 boolean fair) {
      super(fair);
