 ...
 * must be implemented differently in GWT. Namely: <ul> <li>Lazily initializes a
 * set of seen exceptions <li>Decrements a counter atomically
 * </ul>
 */
...
abstract class AggregateFutureState {
  ...
  private static final Logger log =
      Logger.getLogger(AggregateFutureState.class.getName());
  ...
  final Set<Throwable> getOrInitSeenExceptions() {
     ...
     * like. The simple approach would be for each caller CAS it from null to a
     * Set populated with its exception. But there's another race: If the first
     * thread fails with an exception and a second thread immediately fails with
     * the same exception:
     *
     * Thread1: calls setException(), which returns true, context switch before
     * it can CAS seenExceptions to its exception
     *
     * Thread2: calls setException(), which returns false, CASes seenExceptions
     * to its exception, and wrongly believes that its exception is new (leading
     * it to logging it when it shouldn't)
     *
     ...
     * work. This ensures that seenExceptions always contains not just the
     * current thread's exception but also the initial thread's.
     */
    Set<Throwable> seenExceptionsLocal = seenExceptions;
    if (seenExceptionsLocal == null) {
      seenExceptionsLocal = newConcurrentHashSet();
      /*
       * Other handleException() callers may see this as soon as we publish it.
       * We need to populate it with the initial failure before we do, or else
       * they may think that the initial failure has never been seen before.
       */
       ...
       *
       * This read is guaranteed to get us the right value because we only set
       * this once (here).
       */
  ...
  /**
   * Populates {@code seen} with the exception that was passed to {@code
   * setException}.
   */
  ...
  private abstract static class AtomicHelper {
    /**
     * Atomic compare-and-set of the {@link
