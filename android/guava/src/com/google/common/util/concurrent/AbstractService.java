...
public abstract class AbstractService implements Service {
  ...
  private final class IsStartableGuard extends Guard {
    IsStartableGuard() { super(AbstractService.this.monitor); }
  ...
  private final class IsStoppableGuard extends Guard {
    IsStoppableGuard() { super(AbstractService.this.monitor); }
  ...
  private final class HasReachedRunningGuard extends Guard {
    HasReachedRunningGuard() { super(AbstractService.this.monitor); }
  ...
  private final class IsStoppedGuard extends Guard {
    IsStoppedGuard() { super(AbstractService.this.monitor); }
   ...
   * field. This is desirable so that methods like {@link #state}, {@link
   * #failureCause} and notably {@link #toString} can be run without grabbing
   * the lock.
   *
   * <p>To update this field correctly the lock must be held to guarantee that
  ...
  /**
   * This method is called by {@link #startAsync} to initiate service startup.
   * The invocation of this method should cause a call to {@link
   * #notifyStarted()}, either during this method's run, or after it has
   * returned. If startup fails, the invocation should cause a call to {@link
   ...
   * startup, even when {@link #startAsync} is called multiple times.
   */
   ...
   * <p>This method should return promptly; prefer to do work on a different
   * thread where it is convenient. It is invoked exactly once on service
   * shutdown, even when {@link #stopAsync} is called multiple times.
   */
  ...
  throws TimeoutException {
    if (monitor.enterWhenUninterruptibly(hasReachedRunning, timeout, unit)) {
      ...
      // It is possible due to races the we are currently in the expected state
      // even though we timed out. e.g. if we weren't event able to grab the
      // lock within the timeout we would never even check the guard. I don't
  ...
  protected final void notifyStarted() {
    ...
    try {
      ...
      if (snapshot.shutdownWhenStartupFinishes) {
        ...
        // the shutdownWhenStartupFinishes flag.
        doStop();
      } else {
  ...
  protected final void notifyStopped() {
    monitor.enter();
    try {
      // We check the internal state of the snapshot instead of state() directly
      // so we don't allow notifyStopped() to be called while STARTING, even if
   ...
   * The service will <b>not be stopped</b> if it is running. Invoke this method
   * when a service has failed critically or otherwise cannot be started nor
   * stopped.
   */
  ...
  /**
   * Attempts to execute all the listeners in {@link #listeners} while not
   * holding the {@link #monitor}.
   */
  ...
  private void enqueueStartingEvent() { listeners.enqueue(STARTING_EVENT); }

  private void enqueueRunningEvent() { listeners.enqueue(RUNNING_EVENT); }
   ...
   * represents a consistent snapshot of the state and therefore it can be used
   * to answer simple queries without needing to grab a lock.
   */
  @Immutable
