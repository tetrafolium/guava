 ...
 * instantiating a {@link SettableFuture}, submitting a task to a {@link
 * ListeningExecutorService}, and deriving a {@code Future} from an existing
 * one, typically using methods like {@link Futures#transform(ListenableFuture,
 * com.google.common.base.Function, java.util.concurrent.Executor)
 * Futures.transform} and {@link Futures#catching(ListenableFuture, Class,
 * com.google.common.base.Function, java.util.concurrent.Executor)
 * Futures.catching}.
 *
 * <p>This class implements all methods in {@code ListenableFuture}. Subclasses
 ...
 * {@link #afterDone()}, which will be invoked automatically when the future
 * completes. Subclasses should rarely override other methods.
 *
 * @author Sven Mawson
...
public abstract class AbstractFuture<V> extends FluentFuture<V> {
  ...
  // spin instead of blocking. This value is what AbstractQueuedSynchronizer
  // uses.
  ...
  static {
    ...
    try {
      ...
      // be AbstractFuture instead of SafeAtomicHelper, so we annoyingly define
      // these here
      try {
        ...
        // field is definitely there. For these users fallback to a suboptimal
        // implementation, based on synchronized. This will be a definite
        // performance hit to those users.
    ...
    @SuppressWarnings("unused") Class<?> ensureLoaded = LockSupport.class;

    // Log after all static init is finished; if an installed logger uses any
    // Futures methods, it shouldn't break in cases where reflection is
    // missing/broken.
  ...
  private static final class Waiter {
     ...
     * class is loaded before the ATOMIC_HELPER. Apparently this is possible on
     * some android platforms.
     */
    ...
    void unpark() {
      // This is racy with removeWaiter. The consequence of the race is that we
      // may spuriously call unpark even though the thread has already removed
      // itself from the list. But even if we did use a CAS, that race would
      // still exist (it would just be ever so slightly smaller).
      Thread w = thread;
      if (w != null) {
        thread = null;
   ...
   * O(n^2) in the worst), but we are saved by two things. <ul> <li>This is only
   * called when a waiting thread times out or is interrupted. Both of which
   * should be rare. <li>The waiters list should be very short.
   * </ul>
   */
  ...
  restart:
    while (true) {
      ...
      while (curr != null) {
        succ = curr.next;
        if (curr.thread !=
            null) { // we aren't unlinking this node, update pred.
          pred = curr;
        ...
        } else if (!ATOMIC_HELPER.casWaiters(this, curr,
                                             succ)) { // We are unlinking head
  ...
  private static final class Failure {
    ...
    Failure(Throwable exception) { this.exception = checkNotNull(exception); }
  }
  ...
  // TODO(lukes): investigate using the @Contended annotation on these fields
  // when jdk8 is available.
  /**
  ...
  // latency overheads (I
  //   have observed 12 micros on 64 bit linux systems to wake up a parked
  //   thread). So if the timeout is small we shouldn't park(). This needs to be
  //   traded off with the cpu overhead of spinning, so we use
  ...
  //   system scheduling and as such we could either miss our deadline, or
  //   unpark() could be delayed so that it looks like we timed out even though
  //   we didn't. For comparison FutureTask respects completion preferably and
  //   AQS is non-deterministic (depends on where in the queue the waiter is).
  //   If we wanted to be strict about it, we could store the unpark() time in
  //   the Waiter node and we could use that to make a decision about whether or
  //   not we timed out prior to being unparked.

  /**
   * {@inheritDoc}
   *
   * <p>The default {@link AbstractFuture} implementation throws {@code
   * InterruptedException} if the current thread is interrupted during the call,
  ...
  public V get(long timeout, TimeUnit unit)
    ...
    long remainingNanos =
        unit.toNanos(timeout); // we rely on the implicit null check on unit.
    ...
    // spin
    final long endNanos =
        remainingNanos > 0 ? System.nanoTime() + remainingNanos : 0;
  long_wait_loop:
    ...
    // It's confusing to see a completed future in a timeout message; if
    // isDone() returns false, then we know it must have given a pending
    // toString value earlier. If not, then the future completed after the
    // timeout expired, and the message might be success.
    if (isDone()) {
    ...
    throw new TimeoutException("Waited " + timeout + " " +
                               Ascii.toLowerCase(unit.toString()) + " for " +
                               futureToString);
  }
  ...
  /**
   * Unboxes {@code obj}. Assumes that obj is not {@code null} or a {@link
   * SetFuture}.
   */
   ...
   * cancellation will also be propagated to the delegate
   * {@code Future} that was supplied in the {@code setFuture} call.
   *
   * <p>Rather than override this method to perform additional cancellation work
   * or cleanup, subclasses should override {@link #afterDone}, consulting
   * {@link #isCancelled} and {@link #wasInterrupted} as necessary. This ensures
   * that the work is done even if the future is cancelled without a call to
   * {@code cancel}, such as by calling {@code setFuture(cancelledFuture)}.
   */
  @CanIgnoreReturnValue
  @Override
  public boolean cancel(boolean mayInterruptIfRunning) {
    ...
    if (localValue == null | localValue instanceof SetFuture) {
      // Try to delay allocating the exception. At this point we may still lose
      // the CAS, but it is certainly less likely.
      Object valueToSet =
      ...
      while (true) {
        if (ATOMIC_HELPER.casValue(abstractFuture, localValue, valueToSet)) {
          rValue = true;
          // We call interuptTask before calling complete(), which is consistent
          // with FutureTask
          ...
          if (localValue instanceof SetFuture) {
            ...
            if (futureToPropagateTo instanceof TrustedFuture) {
              ...
              // TrustedFuture.cancel is final and does nothing but delegate to
              // this method.
   ...
   *
   * <p>This method is likely to be deprecated. Prefer to override {@link
   ...
   * accepted (in which case it returns {@code true}). If it returns {@code
   * false}, the {@code Future} may have previously been set asynchronously, in
   * which case its result may not be known yet. That result, though not yet
   * known, cannot be overridden by a call to a {@code set*} method, only by a
   * call to {@link #cancel}.
   *
   ...
   * has already been cancelled or set (including {@linkplain #setFuture set
   * asynchronously}). When a call to this method returns, the {@code Future} is
   * guaranteed to be {@linkplain #isDone done} <b>only if</b> the call was
   * accepted (in which case it returns {@code true}). If it returns {@code
   * false}, the
   * {@code Future} may have previously been set asynchronously, in which case
   * its result may not be known yet. That result, though not yet known, cannot
   * be overridden by a call to a {@code set*} method, only by a call to {@link
   * #cancel}.
   *
  ...
  /**
   * Sets the result of this {@code Future} to match the supplied input {@code
   * Future} once the supplied {@code Future} is done, unless this {@code
   * Future} has already been cancelled or set (including "set asynchronously,"
   * defined below).
   *
   * <p>If the supplied future is {@linkplain #isDone done} when this method is
   * called and the call is accepted, then this future is guaranteed to have
   * been completed with the supplied future by the time this method returns. If
   * the supplied future is not done and the call is accepted, then the future
   * will be <i>set asynchronously</i>. Note that such a result, though not yet
   * known, cannot be overridden by a call to a {@code set*} method, only by a
   * call to {@link #cancel}.
   *
   ...
   * delegate}. Additionally, any call to
   * {@code setFuture} after any cancellation will propagate cancellation to the
   * supplied {@code Future}.
   *
   * <p>Note that, even if the supplied future is cancelled and it causes this
   * future to complete, it will never trigger interruption behavior. In
   * particular, it will not cause this future to invoke the {@link
   * #interruptTask} method, and the {@link #wasInterrupted} method will not
   * return {@code true}.
   *
  ...
  protected boolean setFuture(ListenableFuture<? extends V> future) {
    ...
    if (localValue == null) {
      ...
      if (ATOMIC_HELPER.casValue(this, null, valueToSet)) {
        ...
        // directExecutor is appropriate since all we are doing is unpacking a
        // completed future which should be fast.
        try {
          future.addListener(valueToSet, directExecutor());
        } catch (Throwable t) {
          // addListener has thrown an exception! SetFuture.run can't throw any
          // exceptions so this must have been caused by addListener itself. The
          // most likely explanation is a misconfigured mock. Try to switch to
          // Failure.
          Failure failure;
          try {
  ...
  /**
   * Returns a value that satisfies the contract of the {@link #value} field
   * based on the state of given future.
   *
  ...
  private static Object getFutureValue(ListenableFuture<?> future) {
    ...
    if (future instanceof TrustedFuture) {
      ...
      // this is equivalent to calling .get() and unpacking the exceptions like
      // we do below (just much faster because it is a single field read instead
      // of a read, several branches and possibly creating exceptions).
      Object v = ((AbstractFuture<?>)future).value;
      if (v instanceof Cancellation) {
        // If the other future was interrupted, clear the interrupted bit while
        // preserving the cause this will make it consistent with how
        // non-trustedfutures work which cannot propagate the wasInterrupted bit
  ...
  outer:
    while (true) {
      ...
      // separate stack data structure for them. afterDone() should be generally
      // fast and only used for cleanup work... but in theory can also be
      // recursive and create StackOverflowErrors
      future.afterDone();
      ...
      while (next != null) {
        ...
        if (task instanceof SetFuture) {
          ...
          // the case of long chains of SetFutures Handling this special case is
          // important because there is no way to pass an executor to setFuture,
          // so a user couldn't break the chain by doing this themselves.  It is
          // also potentially common if someone writes a recursive
   ...
   *
   * <p>If {@link #interruptTask} is also run during completion, {@link
   * #afterDone} runs after it.
   *
   * <p>The default implementation of this method in {@code AbstractFuture} does
   * nothing. This is intended for very lightweight cleanup work, for example,
   * timing statistics or clearing fields. If your task does anything heavier
   * consider, just using a listener with an executor.
   *
   * @since 20.0
   */
  @Beta
  ...
  /**
   * Returns the exception that this {@code Future} completed with. This
   * includes completion through a call to {@link #setException} or {@link
   * #setFuture setFuture}{@code (failedFuture)} but not cancellation.
   *
   * @throws RuntimeException if the {@code Future} has not failed
   */
  final Throwable trustedGetException() { return ((Failure)value).exception; }
  ...
  // TODO(user) move this up into FluentFuture, or parts as a default method on
  // ListenableFuture?
  @Override
  ...
  protected String pendingToString() {
    ...
    if (localValue instanceof SetFuture) {
      ...
      return "remaining delay=[" +
          ((ScheduledFuture)this).getDelay(TimeUnit.MILLISECONDS) + " ms]";
    }
    return null;
  }
  ...
  private abstract static class AtomicHelper {
    ...
    abstract boolean casWaiters(AbstractFuture<?> future, Waiter expect,
                                Waiter update);

    /** Performs a CAS operation on the {@link #listeners} field. */
    abstract boolean casListeners(AbstractFuture<?> future, Listener expect,
                                  Listener update);

    /** Performs a CAS operation on the {@link #value} field. */
    abstract boolean casValue(AbstractFuture<?> future, Object expect,
                              Object update);
  }
   ...
   *
   * <p>Static initialization of this class will fail if the {@link
   * sun.misc.Unsafe} object cannot be accessed.
   */
  private static final class UnsafeAtomicHelper extends AtomicHelper {
    ...
    static {
      ...
      try {
        ...
        VALUE_OFFSET =
            unsafe.objectFieldOffset(abstractFuture.getDeclaredField("value"));
    ...
    boolean casWaiters(AbstractFuture<?> future, Waiter expect, Waiter update) {
      return UNSAFE.compareAndSwapObject(future, WAITERS_OFFSET, expect,
                                         update);
    }

    /** Performs a CAS operation on the {@link #listeners} field. */
    @Override
    boolean casListeners(AbstractFuture<?> future, Listener expect,
                         Listener update) {
      return UNSAFE.compareAndSwapObject(future, LISTENERS_OFFSET, expect,
                                         update);
    }
  ...
  cancellationExceptionWithCause(@Nullable String message,
