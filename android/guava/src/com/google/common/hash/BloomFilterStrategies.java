 ...
 */
enum BloomFilterStrategies implements BloomFilter
.Strategy {
  /**
  ...
  ,
       ...
       * hashing. It looks different than the implementation in
       * MURMUR128_MITZ_32 because we're avoiding the multiplication in the loop
       * and doing a (much simpler) += hash2. We're also changing the index to a
       * positive number by AND'ing with Long.MAX_VALUE instead of flipping the
       * bits.
       */
      MURMUR128_MITZ_64() {
    @Override
    public <T> boolean put(T object, Funnel<? super T> funnel,
      ...
      long bitSize = bits.bitSize();
      byte[] bytes =
          Hashing.murmur3_128().hashObject(object, funnel).getBytesInternal();
   ...
   *
   * <p>We use this instead of java.util.BitSet because we need access to the
  ...
  static final class LockFreeBitArray {
    ...
    LockFreeBitArray(long bits) {
      this(new long[Ints.checkedCast(
     ...
     * state of the bit array. This is usually good enough, but should be kept
     * in mind.
     */
    ...
    /** Number of bits */
    long bitSize() { return (long)data.length() * Long.SIZE; }

    /**
     * Number of set bits (1s).
     *
     * <p>Note that because of concurrent set calls and uses of atomics, this
     ...
     * that the number, if not exactly accurate, is *always* underestimating,
     * never overestimating.
     */
    long bitCount() { return bitCount.sum(); }

    LockFreeBitArray copy() { return new LockFreeBitArray(toPlainArray(data)); }

    /**
     * Combines the two BitArrays using bitwise OR.
     *
     * <p>NOTE: Because of the use of atomics, if the other LockFreeBitArray is
     * being mutated while this operation is executing, not all of those new 1's
     * may be set in the final state of this LockFreeBitArray. The ONLY
    ...
    void putAll(LockFreeBitArray other) {
      checkArgument(data.length() == other.data.length(),
