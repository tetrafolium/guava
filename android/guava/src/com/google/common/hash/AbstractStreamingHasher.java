...
abstract class AbstractStreamingHasher extends AbstractHasher {
   ...
   *
   * @param chunkSize the number of bytes available per {@link
   *     #process(ByteBuffer)} invocation; must be at least 4
   */
  ...
  protected AbstractStreamingHasher(int chunkSize, int bufferSize) {
    // TODO(kevinb): check more preconditions (as bufferSize >= chunkSize) if
    // this is ever public
    ...
    this.buffer =
        ByteBuffer.allocate(bufferSize + 7).order(ByteOrder.LITTLE_ENDIAN);
   ...
   * non-empty.
   *
   * <p>This implementation simply pads with zeros and delegates to {@link
   * #process(ByteBuffer)}.
   */
   ...
   * to the buffer (or even to an intermediate buffer) should be considerably
   * more efficient than potentially copying the CharSequence to a String and
   * then calling getBytes(Charset) on that String, in reality there are
   * optimizations that make the getBytes(Charset) approach considerably faster,
   ...
   * and any leftover bytes that did not make a complete chunk are handled with
  ...
  private void munchIfFull() {
    if (buffer.remaining() < 8) {
      // buffer is full; not enough room for a primitive. We have at least one
      // full chunk.
      munch();
    }
  }
