...
final class LittleEndianByteArray {
  ...
  /**
   * The instance that actually does the work; delegates to Unsafe or a
   * pure-Java fallback.
   */
  private static final LittleEndianBytes byteArray;

  /**
   * Load 8 bytes into long in a little endian manner, from the substring
   * between position and position + 8. The array must have at least 8 bytes
  ...
  static long load64(byte[] input, int offset) {
    // We don't want this in production code as this is the most critical part
    // of the loop.
  ...
  /**
   * Similar to load64, but allows offset + 8 > input.length, padding the result
   * with zeroes. This has to explicitly reverse the order of the bytes as it
   * packs them into the result which makes it slower than the native version.
   *
   * @param input the input bytes
  ...
  static long load64Safely(byte[] input, int offset, int length) {
    long result = 0;
    // Due to the way we shift, we can stop iterating once we've run out of
    // data, the rest of the result already being filled with zeros.

    // This loop is critical to performance, so please check HashBenchmark if
    // altering it.
  ...
  /**
   * Store 8 bytes into the provided array at the indicated offset, using the
   * value provided.
   *
   ...
   * operations will be very efficient. May be useful for calling code to fall
   * back on an alternative implementation that is slower than Unsafe.get/store
   * but faster than the pure-Java mask-and-shift.
   */
  ...
  /**
   * Common interface for retrieving a 64-bit long from a little-endian byte
   * array.
   *
   * This abstraction allows us to use single-instruction load and put when
   * available, or fall back on the slower approach of using
   * Longs.fromBytes(byte...).
   */
  ...
  /**
   * The only reference to Unsafe is in this nested class. We set things up so
   * that if Unsafe.theUnsafe is inaccessible, the attempt to load the nested
   * class fails, and the outer class's static initializer can fall back on a
   * non-Unsafe version.
   */
  private enum UnsafeByteArray implements LittleEndianBytes {
    ...
    UNSAFE_BIG_ENDIAN {
      ...
      public long getLongLittleEndian(byte[] array, int offset) {
        long bigEndian =
            theUnsafe.getLong(array, (long)offset + BYTE_ARRAY_BASE_OFFSET);
    ...
    // Provides load and store operations that use native instructions to get
    // better performance.
