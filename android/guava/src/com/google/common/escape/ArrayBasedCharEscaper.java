 ...
 * {@code char} value. An additional safe range is provided that determines
 * whether {@code char} values without specific replacements are to be
 ...
 * characters such as {@code \\t} and {@code \\n} while {@link #escapeUnsafe} is
 * overridden to handle general escaping of the form {@code \\uxxxx}.
 *
 ...
 * example a replacement map containing the single character '{@code \}{@code
 * u1000}' will require approximately 16K of memory. If you need to create
 * multiple escaper instances that have the same character replacement mapping
 * consider using {@link ArrayBasedEscaperMap}.
 *
 * @author Sven Mawson
...
public abstract class ArrayBasedCharEscaper extends CharEscaper {
   ...
   * safe range. If it lies outside that, then {@link #escapeUnsafe} is called,
  ...
  public final String escape(String s) {
    ...
    for (int i = 0; i < s.length(); i++) {
      char c = s.charAt(i);
      if ((c < replacementsLength && replacements[c] != null) || c > safeMax ||
          c < safeMin) {
   ...
   * lies outside the safe range then
