...
public final class BigIntegerMath {
  ...
  public static int log10(BigInteger x, RoundingMode mode) {
    ...
    if (approxCmp > 0) {
      /*
       * The code is written so that even completely incorrect approximations
       * will still yield the correct answer eventually, but in practice this
       * branch should almost never be entered, and even then the loop should
    ...
    case HALF_UP:
    case HALF_EVEN:
      // Since sqrt(10) is irrational, log10(x) - floorLog can never be exactly
      // 0.5
      BigInteger x2 = x.pow(2);
  ...
  public static BigInteger sqrt(BigInteger x, RoundingMode mode) {
    ...
    case HALF_EVEN:
       ...
       */
      return (halfSquare.compareTo(x) >= 0) ? sqrtFloor
                                            : sqrtFloor.add(BigInteger.ONE);
    default:
      throw new AssertionError();
    }
  }
  ...
  private static BigInteger sqrtFloor(BigInteger x) {
     ...
     * root. Then, we iteratively improve this guess with an application of
     * Newton's method, which sets guess := (guess + (x / guess)) / 2. This
     * iteration has the following two properties:
     *
     * a) every iteration (except potentially the first) has guess >=
     * floor(sqrt(x)). This is because guess' is the arithmetic mean of guess
     * and x / guess, sqrt(x) is the geometric mean, and the arithmetic mean is
     * always higher than the geometric mean.
     *
     ...
     *
     * We start out with a double-precision approximation, which may be higher
     * or lower than the true value. Therefore, we perform at least one Newton
  ...
  public static BigInteger binomial(int n, int k) {
    ...
    if (k < LongMath.biggestBinomials.length &&
        n <= LongMath.biggestBinomials[k]) {
