...
public final class Longs {
   ...
   * in java, which might be different from {@code ((Long) value).hashCode()} in
   * GWT because
...
outer:
   ...
   * For example,
   * {@code concat(new long[] {a, b}, new long[] {}, new long[] {c}} returns the
   * array
   * {@code {a, b, c}}.
   *
   ...
   * input value
   * {@code 0x1213141516171819L} would yield the byte array {@code {0x12, 0x13,
   ...
   * ByteBuffer.wrap(bytes).getLong()}. For example, the input byte array {@code
  ...
  /*
   * Moving asciiDigits into this static holder class lets ProGuard eliminate
   * and inline the Longs class.
   */
  static final class AsciiDigits {
    ...
    static int digit(char c) { return (c < 128) ? asciiDigits[c] : -1; }
  }
   ...
   *
   * <p>Unlike {@link Long#parseLong(String)}, this method returns {@code null}
   * instead of throwing an exception if parsing fails. Additionally, this
   * method only accepts ASCII digits, and returns
   * {@code null} if non-ASCII digits are present in the string.
   *
   * <p>Note that strings prefixed with ASCII {@code '+'} are rejected, even
   * under JDK 7, despite the change to {@link Long#parseLong(String)} for that
   * version.
   *
   ...
   *
   * <p>Unlike {@link Long#parseLong(String, int)}, this method returns {@code
   * null} instead of throwing an exception if parsing fails. Additionally, this
   * method only accepts ASCII digits, and returns {@code null} if non-ASCII
   * digits are present in the string.
   *
   * <p>Note that strings prefixed with ASCII {@code '+'} are rejected, even
   * under JDK 7, despite the change to {@link Long#parseLong(String, int)} for
   * that version.
   *
   * @param string the string representation of an long value
   * @param radix the radix to use when parsing
   * @return the long value represented by {@code string} using {@code radix},
   *     or {@code null} if
   *     {@code string} has a length of zero or cannot be parsed as a long value
   ...
   * longs using
   * {@link Long#decode} and {@link Long#toString()}. The returned converter
   * throws
   * {@link NumberFormatException} if the input string is invalid.
   *
   * <p><b>Warning:</b> please see {@link Long#decode} to understand exactly how
   * strings are parsed. For example, the string {@code "0123"} is treated as
   * <i>octal</i> and converted to the value
   * {@code 83L}.
   *
   * @since 16.0
   */
   ...
   * Collections.reverse(Longs.asList(array).subList(fromIndex, toIndex))}, but
   ...
   * ImmutableLongArray} instead, which has an {@link ImmutableLongArray#asList
   * asList} view.
   *
  ...
  private static class LongArrayAsList
    ...
    LongArrayAsList(long[] array) { this(array, 0, array.length); }
    ...
    long[] toLongArray() { return Arrays.copyOfRange(array, start, end); }
