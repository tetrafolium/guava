...
public final class Chars {
   ...
   *     {@code char} type,
   *     {@link Character#MAX_VALUE} if it is too large, or {@link
...
outer:
   ...
   * For example,
   * {@code concat(new char[] {a, b}, new char[] {}, new char[] {c}} returns the
   * array
   * {@code {a, b, c}}.
   *
   ...
   * ByteBuffer.wrap(bytes).getChar()}. For example, the input byte array {@code
   * {0x54, 0x32}} would yield the {@code char} value {@code '\\u5432'}.
   *
   ...
   * Collections.reverse(Chars.asList(array).subList(fromIndex, toIndex))}, but
  ...
  private static class CharArrayAsList
    ...
    CharArrayAsList(char[] array) { this(array, 0, array.length); }
    ...
    char[] toCharArray() { return Arrays.copyOfRange(array, start, end); }
