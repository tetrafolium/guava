 ...
 * {@code 2^64 + x}). The methods for which signedness is not an issue are in
 * {@link Longs}, as well as signed versions of methods for which signedness is
 * an issue.
 *
 * <p>In addition, this class provides several static methods for converting a
 * {@code long} to a
 * {@code String} and a {@code String} to a {@code long} that treat the {@code
 * long} as an unsigned number.
 *
 * <p>Users of these utilities must be <i>extremely careful</i> not to mix up
 * signed and unsigned
 * {@code long} values. When possible, it is recommended that the {@link
 * UnsignedLong} wrapper class be used, at a small efficiency penalty, to
...
public final class UnsignedLongs {
   ...
   * the ordering on longs, that is, {@code a <= b} as unsigned longs if and
   * only if {@code flip(a) <= flip(b)} as signed longs.
   */
  private static long flip(long a) { return a ^ Long.MIN_VALUE; }
   ...
   *
   * <p><b>Java 8 users:</b> use {@link Long#compareUnsigned(long, long)}
   * instead.
   *
   ...
   * That is, it compares, using {@link #compare(long, long)}), the first pair
   ...
   * exclusive, treating its elements as unsigned 64-bit integers.
   *
   * @since 23.1
   */
   ...
   *
   * <p><b>Java 8 users:</b> use {@link Long#divideUnsigned(long, long)}
   * instead.
   *
  ...
  public static long divide(long dividend, long divisor) {
     ...
     * approximation is guaranteed to be either exact or one less than the
   ...
   * unsigned 64-bit quantities.
   *
   * <p><b>Java 8 users:</b> use {@link Long#remainderUnsigned(long, long)}
   * instead.
   *
   * @param dividend the dividend (numerator)
   * @param divisor the divisor (denominator)
  ...
  public static long remainder(long dividend, long divisor) {
     ...
     * Otherwise, approximate the quotient, check, and correct if necessary. Our
     * approximation is guaranteed to be either exact or one less than the
     * correct value. This follows from the fact that floor(floor(x)/i) ==
     * floor(x/i) for any real x and integer i != 0. The proof is not quite
     * trivial.
     */
    long quotient = ((dividend >>> 1) / divisor) << 1;
   ...
   * given radix.
   *
   * <p><b>Java 8 users:</b> use {@link Long#parseUnsignedLong(String, int)}
   * instead.
   *
   * @param string the string containing the unsigned {@code long}
   *     representation to be parsed.
   * @param radix the radix to use while parsing {@code string}
   * @throws NumberFormatException if the string does not contain a valid
   *     unsigned {@code long} with the given radix, or if {@code radix} is not
   *     between {@link Character#MIN_RADIX} and
  ...
  /*
   * We move the static constants into this class so ProGuard can inline
   * UnsignedLongs entirely unless the user is actually calling a parse method.
   */
   ...
   */
  public static String toString(long x) { return toString(x, 10); }

  /**
   * Returns a string representation of {@code x} for the given radix, where
   * {@code x} is treated as unsigned.
   *
   * <p><b>Java 8 users:</b> use {@link Long#toUnsignedString(long, int)}
   * instead.
   *
