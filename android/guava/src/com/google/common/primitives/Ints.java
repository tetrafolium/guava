...
outer:
   ...
   * For example,
   * {@code concat(new int[] {a, b}, new int[] {}, new int[] {c}} returns the
   * array {@code {a, b, c}}.
   *
   ...
   * input value
   * {@code 0x12131415} would yield the byte array {@code {0x12, 0x13, 0x14,
   * 0x15}}.
   *
   ...
   * {@code bytes}; equivalent to {@code ByteBuffer.wrap(bytes).getInt()}. For
   * example, the input byte array {@code {0x12, 0x13, 0x14, 0x15, 0x33}} would
   * yield the {@code int} value
   * {@code 0x12131415}.
   *
   ...
   * integers using
   * {@link Integer#decode} and {@link Integer#toString()}. The returned
   * converter throws
   * {@link NumberFormatException} if the input string is invalid.
   *
   * <p><b>Warning:</b> please see {@link Integer#decode} to understand exactly
   ...
   * Collections.reverse(Ints.asList(array))}, but is likely to be more
   * efficient.
   *
   * @since 23.1
   */
   ...
   * Collections.reverse(Ints.asList(array).subList(fromIndex, toIndex))}, but
   ...
   * ImmutableIntArray} instead, which has an {@link ImmutableIntArray#asList
   * asList} view.
   *
  ...
  private static class IntArrayAsList
    ...
    IntArrayAsList(int[] array) { this(array, 0, array.length); }
    ...
    int[] toIntArray() { return Arrays.copyOfRange(array, start, end); }
   ...
   *
   * <p>Unlike {@link Integer#parseInt(String)}, this method returns {@code
   ...
   *
   * <p>Unlike {@link Integer#parseInt(String, int)}, this method returns {@code
   * null} instead of throwing an exception if parsing fails. Additionally, this
   * method only accepts ASCII digits, and returns {@code null} if non-ASCII
   * digits are present in the string.
   *
   ...
   * @return the integer value represented by {@code string} using {@code
   *     radix}, or {@code null} if
   *     {@code string} has a length of zero or cannot be parsed as an integer
   * value
