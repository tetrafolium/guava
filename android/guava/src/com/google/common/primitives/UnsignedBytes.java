 ...
 * {@code 256 + b}). The corresponding methods that treat the values as signed
 * are found in
 * {@link SignedBytes}, and the methods for which signedness is not an issue are
 * in {@link Bytes}.
 *
...
public final class UnsignedBytes {
   ...
   * {@code value + 256} if {@code value} is negative; {@code value} itself
   * otherwise.
   *
   ...
   * @since 6.0
   */
  public static int toInt(byte value) { return value & UNSIGNED_MASK; }
   ...
   * given radix.
   *
   * @param string the string containing the unsigned {@code byte}
   *     representation to be parsed.
   * @param radix the radix to use while parsing {@code string}
   * @throws NumberFormatException if the string does not contain a valid
   *     unsigned {@code byte} with the given radix, or if {@code radix} is not
   ...
   * {@code separator}. For example, {@code join(":", (byte) 1, (byte) 2, (byte)
   * 255)} returns the string {@code "1:2:255"}.
   *
  ...
  public static String join(String separator, byte... array) {
    ...
    StringBuilder builder =
        new StringBuilder(array.length * (3 + separator.length()));
   ...
   * [0x01] < [0x01, 0x7F] < [0x01, 0x80] < [0x02]}. Values are treated as
   * unsigned.
   *
  ...
  static class LexicographicalComparatorHolder {
    ...
    @VisibleForTesting enum UnsafeComparator implements Comparator < byte
       ...
       * and can constant-fold them while (final or not) local variables are run
       * time values.
       */
   ...
   * exclusive, treating its elements as unsigned bytes.
   *
   * @since 23.1
   */
