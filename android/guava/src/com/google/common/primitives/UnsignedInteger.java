 ...
 * operations.
 *
 * <p>In some cases, when speed is more important than code readability, it may
 * be faster simply to treat primitive {@code int} values as unsigned, using the
...
public final class UnsignedInteger
  ...
  private UnsignedInteger(int value) {
    // GWT doesn't consistently overflow values to make them 32-bit, so we need
    // to force it.
   ...
   *
   * <p>If the argument is nonnegative, the returned result will be equal to
   * {@code bits}, otherwise, the result will be equal to {@code 2^32 + bits}.
   *
   * <p>To represent unsigned decimal constants, consider {@link #valueOf(long)}
   * instead.
   *
   * @since 14.0
   */
   ...
   * specified
   * {@link BigInteger}. This is the inverse operation of {@link
   * #bigIntegerValue()}.
   *
   * @throws IllegalArgumentException if {@code value} is negative or {@code
   *     value >= 2^32}
   */
  ...
  /**
   * Returns the value of this {@code UnsignedInteger} as an {@code int}. This
   * is an inverse operation to {@link #fromIntBits}.
   *
   * <p>Note that if this {@code UnsignedInteger} holds a value {@code >= 2^31},
   ...
   * analogous to a widening primitive conversion from {@code int} to {@code
  ...
  /**
   * Compares this unsigned integer to another unsigned integer. Returns {@code
   * 0} if they are equal, a negative number if {@code this < other}, and a
   * positive number if
   * {@code this > other}.
   */
  @Override
  ...
  /**
   * Returns a string representation of the {@code UnsignedInteger} value, in
   * base 10.
   */
  @Override
   ...
   * Returns a string representation of the {@code UnsignedInteger} value, in
   * base {@code radix}. If
   * {@code radix < Character.MIN_RADIX} or {@code radix > Character.MAX_RADIX},
   * the radix
   * {@code 10} is used.
   */
