 ...
 * {@code 2^32 + x}). The methods for which signedness is not an issue are in
 * {@link Ints}, as well as signed versions of methods for which signedness is
 * an issue.
 *
 * <p>In addition, this class provides several static methods for converting an
 * {@code int} to a
 * {@code String} and a {@code String} to an {@code int} that treat the {@code
 * int} as an unsigned number.
 *
 * <p>Users of these utilities must be <i>extremely careful</i> not to mix up
 * signed and unsigned
 * {@code int} values. When possible, it is recommended that the {@link
 * UnsignedInteger} wrapper class be used, at a small efficiency penalty, to
...
public final class UnsignedInts {
  ...
  static int flip(int value) { return value ^ Integer.MIN_VALUE; }
   ...
   *
   * <p><b>Java 8 users:</b> use {@link Integer#compareUnsigned(int, int)}
   * instead.
   *
  ...
  /**
   * Returns the value of the given {@code int} as a {@code long}, when treated
   * as unsigned.
   *
   * <p><b>Java 8 users:</b> use {@link Integer#toUnsignedLong(int)} instead.
   */
  public static long toLong(int value) { return value & INT_MASK; }
   ...
   * @return the {@code int} value that, when treated as unsigned, equals {@code
   *     value}
   * @throws IllegalArgumentException if {@code value} is negative or greater
   *     than or equal to 2<sup>32</sup>
   * @since 21.0
   */
  ...
  /**
   * Returns the {@code int} value that, when treated as unsigned, is nearest in
   * value to
   * {@code value}.
   *
   * @param value any {@code long} value
   * @return {@code 2^32 - 1} if {@code value >= 2^32}, {@code 0} if {@code
   *     value <= 0}, and
  ...
  /**
   * Returns a comparator that compares two arrays of unsigned {@code int}
   * values <a
   ...
   * other, treats the shorter array as the lesser. For example, {@code [] < [1]
   * < [1, 2] < [2] < [1 << 31]}.
   *
  ...
  /**
   * Sorts the array between {@code fromIndex} inclusive and {@code toIndex}
   * exclusive, treating its elements as unsigned 32-bit integers.
   *
   * @since 23.1
   */
  ...
  /**
   * Sorts the elements of {@code array} in descending order, interpreting them
   ...
   * {@code toIndex} exclusive in descending order, interpreting them as
   ...
   *
   * <p><b>Java 8 users:</b> use {@link Integer#divideUnsigned(int, int)}
   * instead.
   *
   ...
   * unsigned 32-bit quantities.
   *
   * <p><b>Java 8 users:</b> use {@link Integer#remainderUnsigned(int, int)}
   * instead.
   *
   * @param dividend the dividend (numerator)
   * @param divisor the divisor (denominator)
   ...
   * string.
   *
   * <p><b>Java 8 users:</b> use {@link Integer#parseUnsignedInt(String)}
   * instead.
   *
   * @throws NumberFormatException if the string does not contain a valid
   *     unsigned {@code int} value
   ...
   * given radix.
   *
   * <p><b>Java 8 users:</b> use {@link Integer#parseUnsignedInt(String, int)}
   * instead.
   *
   * @param string the string containing the unsigned integer representation to
   *     be parsed.
   ...
   */
  public static String toString(int x) { return toString(x, 10); }

  /**
   * Returns a string representation of {@code x} for the given radix, where
   * {@code x} is treated as unsigned.
   *
   * <p><b>Java 8 users:</b> use {@link Integer#toUnsignedString(int, int)}
   * instead.
   *
   * @param x the value to convert to a string.
   * @param radix the radix to use while working with {@code x}
   * @throws IllegalArgumentException if {@code radix} is not between {@link
