...
/**
 * A {@link ValueGraph} whose elements and structural relationships will never
...
public final class ImmutableValueGraph<N, V>
  ...
  public static <N, V> ImmutableValueGraph<N, V>
  ...
  getNodeConnections(ValueGraph<N, V> graph) {
    ...
    // the map will have whatever ordering the graph's nodes do, so
    // ImmutableSortedMap is unnecessary even if the input nodes are sorted.
    ImmutableMap.Builder<N, GraphConnections<N, V>> nodeConnections =
  ...
  connectionsOf(final ValueGraph<N, V> graph, final N node) {
