...
public final class Suppliers {
   ...
   * retrieving the value from {@code supplier}, and then applying {@code
   ...
   * call to {@code get()} and returns that value on subsequent calls to {@code
   * get()}. See: <a
   * href="http://en.wikipedia.org/wiki/Memoization">memoization</a>
   *
   * <p>The returned supplier is thread-safe. The delegate's {@code get()}
   * method will be invoked at most once unless the underlying {@code get()}
   * throws an exception. The supplier's serialized form does not contain the
   * cached value, which will be recalculated when {@code get()} is called on
   * the reserialized instance.
   *
   * <p>When the underlying delegate throws an exception then this memoizing
   * supplier will keep delegating calls until it returns valid data.
   *
   ...
   * get()} is called on the reserialized instance. The actual memoization does
  ...
  private static class ThreadSafeSupplier<T>
    ...
    @Override
    public T get() {
      synchronized (delegate) { return delegate.get(); }
    }
   ...
   *
   * <p><b>Java 8 users:</b> use the method reference {@code Supplier::get}
   * instead.
   *
   * @since 8.0
   */
