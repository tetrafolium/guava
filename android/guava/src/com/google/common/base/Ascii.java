 ...
 * gwt javadoc? --> <li>{@link Charsets#US_ASCII} specifies the {@code Charset}
 * of ASCII characters. <li>{@link CharMatcher#ascii} matches ASCII characters
 * and provides text processing methods which operate only on the ASCII
...
public final class Ascii {
   ...
   *
   * <p>Although RFC 20 names this as "Null", note that it is distinct from the
   * C/C++ "NULL" pointer.
   *
   * @since 8.0
   */
   ...
   * of a sequence of characters which constitute a machine-sensible address or
   * routing information. Such a sequence is referred to as the "heading." An
   * STX character has the effect of terminating a heading.
   *
   * @since 8.0
   */
  ...
  /**
   * Start of Text: A communication control character which precedes a sequence
   * of characters that is to be treated as an entity and entirely transmitted
   * through to the ultimate destination. Such a sequence is referred to as
   * "text." STX may be used to terminate a sequence of characters started by
   * SOH.
   *
   * @since 8.0
   */
   ...
   * conclusion of a transmission, which may have contained one or more texts
   * and any associated headings.
   *
   * @since 8.0
   */
   ...
   * as a "Who Are You" (WRU) to obtain identification, or may be used to obtain
   ...
   * (NL), a format effector which controls the movement of the printing point
   * to the first printing position on the next printing line. Use of this
   ...
   * especially switching devices "on" or "off." (If a single "stop" control is
   * required to interrupt or turn off ancillary devices, DC4 is the preferred
   * assignment.)
   *
   * @since 8.0
   */
   ...
   * indicate the end of a block of data for communication purposes. ETB is used
   * for blocking data where the block structure is not necessarily related to
  ...
  /**
   * End of Medium: A control character associated with the sent data which may
   * be used to identify the physical end of the medium, or the end of the used,
   * or wanted, portion of information recorded on a medium. (The position of
   * this character does not necessarily correspond to the physical end of the
   * medium.)
   *
   * @since 8.0
   */
   ...
   * in optional fashion, except that their hierarchical relationship shall be:
   * FS is the most inclusive, then GS, then RS, and US is least inclusive. (The
  ...
  /**
   * Group Separator: These four information separators may be used within data
   * in optional fashion, except that their hierarchical relationship shall be:
   * FS is the most inclusive, then GS, then RS, and US is least inclusive. (The
   * content and length of a File, Group, Record, or Unit are not specified.)
   *
  ...
  /**
   * Returns a copy of the input string in which all {@linkplain
   * #isUpperCase(char) uppercase ASCII characters} have been converted to
  ...
  /**
   * Returns a copy of the input string in which all {@linkplain
   * #isLowerCase(char) lowercase ASCII characters} have been converted to
  ...
  public static boolean isLowerCase(char c) {
    ...
    // 'a') < 26" (Nov '13) and found to perform at least as well, or better.
    return (c >= 'a') && (c <= 'z');
  }
   ...
   * length of the sequence is greater than {@code maxLength}, the returned
   * string will be exactly
   * {@code maxLength} chars in length and will end with the given {@code
   * truncationIndicator}. Otherwise, the sequence will be returned as a string
   ...
   *
   * <p><b>Note:</b> This method <i>may</i> work with certain non-ASCII text but
   * is not safe for use with arbitrary Unicode text. It is mostly intended for
   * use with text that is known to be safe for use with it (such as all-ASCII
   * text) and for simple debugging text. When using this method, consider the
   * following:
   *
   * <ul>
   ...
   * must be taken into account <li>the appropriate truncation indicator may be
   * locale-dependent <li>it is safe to use non-ASCII characters in the
   * truncation indicator
   * </ul>
   *
   *
   * @throws IllegalArgumentException if {@code maxLength} is less than the
   *     length of
   ...
   * characters between {@code 'a'} and {@code 'z'} or {@code 'A'} and {@code
   * 'Z'} inclusive.
   *
   * <p>This method is significantly faster than {@link String#equalsIgnoreCase}
   * and should be used in preference if at least one of the parameters is known
   * to contain only ASCII characters.
   *
   * <p>Note however that this method does not always behave identically to
   * expressions such as: <ul> <li>{@code string.toUpperCase().equals("UPPER
   * CASE ASCII")} <li>{@code string.toLowerCase().equals("lower case ascii")}
   * </ul>
   * <p>due to case-folding of some non-ASCII characters (which does not occur
   * in
   ...
   * method rather than the subtle and sometimes surprising behavior of {@code
   * toUpperCase()} and {@code toLowerCase()}.
   *
   * @since 16.0
   */
  public static boolean equalsIgnoreCase(CharSequence s1, CharSequence s2) {
    ...
    for (int i = 0; i < length; i++) {
      ...
      // This was also benchmarked using '&' to avoid branching (but always
      // evaluate the rhs), however this showed no obvious improvement.
  ...
  private static int getAlphaIndex(char c) {
    // Fold upper-case ASCII to lower-case and make zero-indexed and unsigned
    // (by casting to char).
