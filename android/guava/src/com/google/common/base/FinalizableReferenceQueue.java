 ...
 *
 * <p>As an example of how this is used, imagine you have a class {@code
 * MyServer} that creates a a
 * {@link java.net.ServerSocket ServerSocket}, and you would like to ensure that
 * the
 * {@code ServerSocket} is closed even if the {@code MyServer} object is
 * garbage-collected without calling its {@code close} method. You
 * <em>could</em> use a finalizer to accomplish this, but that has a number of
 * well-known problems. Here is how you might use this class instead:
 *
 * <pre>   {@code
 * public class MyServer implements Closeable {
 *   private static final FinalizableReferenceQueue frq = new
 ...
 * FinalizablePhantomReference<MyServer>(myServer, frq) { public void
 * finalizeReferent() { references.remove(this): if (!serverSocket.isClosed()) {
...
public class FinalizableReferenceQueue implements Closeable {
  ...
  static class SystemLoader implements FinalizerLoader {
    // This is used by the ClassLoader-leak test in
    // FinalizableReferenceQueueTest to disable finding Finalizer on the system
    // class path even if it is there.
  ...
  static class DecoupledLoader implements FinalizerLoader {
    ...
    URL getBaseUrl() throws IOException {
      ...
      urlString =
          urlString.substring(0, urlString.length() - finalizerPath.length());
      return new URL(finalizerUrl, urlString);
    }

    /** Creates a class loader with the given base URL as its classpath. */
    URLClassLoader newLoader(URL base) {
      // We use the bootstrap class loader as the parent because Finalizer by
      // design uses only standard Java classes. That also means that
      // FinalizableReferenceQueueTest doesn't pick up the wrong version of the
      // Finalizer class.
  ...
  static Method getStartFinalizer(Class<?> finalizer) {
    try {
      return finalizer.getMethod("startFinalizer", Class.class,
                                 ReferenceQueue.class, PhantomReference.class);
