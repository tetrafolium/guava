 ...
 * {@link Map}) with a separator. It either appends the results to an {@link
 * Appendable} or returns them as a {@link String}. Example: <pre>   {@code
 *
 *   Joiner joiner = Joiner.on("; ").skipNulls();
 *    . . .
 *   return joiner.join("Harry", null, "Ron", "Hermione");}</pre>
 *
 * <p>This returns the string {@code "Harry; Ron; Hermione"}. Note that all
 ...
 * <a href="https://github.com/google/guava/wiki/StringsExplained#joiner">{@code
 * Joiner}</a>.
 *
...
public class Joiner {
   ...
   */
  @CanIgnoreReturnValue
  public final <A extends Appendable>
  A appendTo(A appendable, @Nullable Object first, @Nullable Object second,
   ...
   *
   * <p>In addition to operating on {@code Map} instances, {@code MapJoiner} can
   * operate on {@code Multimap} entries in two distinct modes:
   *
   * <ul>
   * <li>To output a separate entry for each key-value pair, pass {@code
   * multimap.entries()} to a
   * {@code MapJoiner} method that accepts entries as input, and receive output
   * of the form
   * {@code key1=A&key1=B&key2=C}.
   * <li>To output a single entry for each key, pass {@code multimap.asMap()} to
   * a {@code MapJoiner} method that accepts a map as input, and receive output
   * of the form {@code key1=[A, B]&key2=C}.
   * </ul>
   *
   * @since 2.0
   */
  public static final class MapJoiner {
     ...
     * {@link #appendTo(Appendable, Map)}, except that it does not throw {@link
     * IOException}.
     */
    @CanIgnoreReturnValue
    ...
    /**
     * Appends the string representation of each entry in {@code entries}, using
     * the previously configured separator and key-value separator, to {@code
     * appendable}.
     *
     * @since 10.0
     */
    ...
    A appendTo(A appendable, Iterable<? extends Entry<?, ?>> entries)
    throws IOException {
    ...
    @CanIgnoreReturnValue
    public <A extends Appendable>
    A appendTo(A appendable, Iterator<? extends Entry<?, ?>> parts)
    throws IOException {
      checkNotNull(appendable);
      if (parts.hasNext()) {
  ...
  CharSequence toString(Object part) {
    ...
    return (part instanceof CharSequence) ? (CharSequence)part
                                          : part.toString();
  }

  private static Iterable<Object>
  iterable(final Object first, final Object second, final Object[] rest) {
    checkNotNull(rest);
