 ...
 * and closed. Instead, it is an immutable <i>supplier</i> of {@code
...
public abstract class ByteSource {
   ...
   *
   * <p>If {@link CharSource#asByteSource} is called on the returned source with
   ...
   * returned stream is not required to be a {@link BufferedInputStream} in
   * order to allow implementations to simply delegate to {@link #openStream()}
   * when the stream returned by that method does not benefit from additional
   * buffering (for example, a {@code ByteArrayInputStream}). This method
   * returns a new, independent stream each time it is called.
   *
   * <p>The caller is responsible for ensuring that the returned stream is
   * closed.
  ...
  /**
   * Returns a view of a slice of this byte source that is at most {@code
   * length} bytes long starting at the given {@code offset}. If {@code offset}
   * is greater than the size of this source, the returned source will be empty.
   * If {@code offset + length} is greater than the size of this source, the
   * returned source will contain the slice starting at {@code offset} and
  ...
  /**
   * Returns whether the source has zero bytes. The default implementation first
   * checks
   * {@link #sizeIfKnown}, returning true if it's known to be zero and false if
   * it's known to be non-zero. If the size is not known, it falls back to
   * opening a stream and checking for EOF.
   *
   * <p>Note that, in cases where {@code sizeIfKnown} returns zero, it is
   * <i>possible</i> that bytes are actually available for reading. (For
   * example, some special files may return a size of 0 despite actually having
   * content when read.) This means that a source may return {@code true} from
   ...
   * sources, such as a file, may return a non-absent value. Note that in such
   * cases, it is <i>possible</i> that this method will return a different
   * number of bytes than would be returned by reading all of the bytes (for
   * example, some special files may return a size of 0 despite actually having
   * content when read).
   *
   * <p>Additionally, for mutable sources such as files, a subsequent read may
   ...
   * and traversing an entire stream. To avoid a potentially expensive
   * operation, see {@link #sizeIfKnown}.
   *
   * <p>The default implementation calls {@link #sizeIfKnown} and returns the
   * value if present. If absent, it will fall back to a heavyweight operation
   * that will open a stream, read (or
   * {@link InputStream#skip(long) skip}, if possible) to the end of the stream
   * and return the total number of bytes that were read.
   *
   * <p>Note that for some sources that implement {@link #sizeIfKnown} to
   * provide a more efficient implementation, it is <i>possible</i> that this
   ...
   * return a different number of bytes if the contents are changed.
   *
   * @throws IOException if an I/O error occurs while reading the size of this
   *     source
   */
   ...
   * SKIP_FAILED if the first call to skip threw, in which case skip may just
   * not be supported.
   */
   ...
   * consumer returns {@code false}. Returns the result produced by the
   * processor.
   *
   * @throws IOException if an I/O error occurs while reading from this source
   *     or if
   ...
   * only a specific range in the array, use {@code
  ...
  class AsCharSource extends CharSource {
    ...
    AsCharSource(Charset charset) { this.charset = checkNotNull(charset); }
    ...
    public String read() throws IOException {
      ...
      // The downside is that this will cause us to store the file bytes in
      // memory twice for a short amount of time.
  ...
  private static class ByteArrayByteSource extends ByteSource {
    ...
    ByteArrayByteSource(byte[] bytes) { this(bytes, 0, bytes.length); }
  ...
  private static final class EmptyByteSource extends ByteArrayByteSource {
    ...
    EmptyByteSource() { super(new byte[0]); }
