 ...
 *
 * <p>This class can't depend on other Guava code. If we were to load this class
 * in the same class loader as the rest of Guava, this thread would keep an
...
public class Finalizer implements Runnable {
  ...
  public static void startFinalizer(Class<?> finalizableReferenceClass,
    ...
    Finalizer finalizer =
        new Finalizer(finalizableReferenceClass, queue, frqReference);
    ...
    if (bigThreadConstructor != null) {
      try {
        ...
        thread = bigThreadConstructor.newInstance((ThreadGroup)null, finalizer,
            threadName, defaultStackSize,
            inheritThreadLocals);
      } catch (Throwable t) {
        logger.log(
          Level.INFO,
  ...
  // By preference, we will use the Thread constructor that has an
  // `inheritThreadLocals` parameter. But before Java 9, our only way not to
  // inherit ThreadLocals is to zap them after the thread is created, by
  ...
  private Finalizer(Class<?> finalizableReferenceClass,
      ReferenceQueue<Object> queue,
      PhantomReference<Object> frqReference) {
    this.queue = queue;
  ...
  private boolean cleanUp(Reference<?> reference) {
    ...
    do {
      ...
      /*
       * Loop as long as we have references available so as not to waste CPU
       * looking up the Method over and over again.
       */
  ...
  private Method getFinalizeReferentMethod() {
    ...
    if (finalizableReferenceClass == null) {
       ...
       * chance that other finalizable references could be enqueued subsequently
       * (at which point the class loader would be resurrected by virtue of us
       * having a strong reference to it), we should pretty much just shut down
       * and make sure we don't keep it alive any longer than necessary.
       */
      return null;
    }
    try {
  ...
  private static Constructor<Thread> getBigThreadConstructor() {
    try {
      return Thread.class.getConstructor(ThreadGroup.class, Runnable.class,
