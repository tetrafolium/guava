 ...
 * reasons. <li>As documented by {@code nanoTime}, the value returned has no
 * absolute meaning, and can only be interpreted as relative to another
 * timestamp returned by {@code nanoTime} at a different time. {@code Stopwatch}
 * is a more effective abstraction because it exposes only these relative
 * values, not the absolute ones.
 * </ul>
 *
 * <p>Basic usage:
 *
 ...
 * #createUnstarted(Ticker)} or {@link #createStarted(Ticker)} to supply a fake
 * or mock ticker. This allows you to simulate any valid behavior of the
 * stopwatch.
 *
 * <p><b>Note:</b> This class is not thread-safe.
 *
 * <p><b>Warning for Android users:</b> a stopwatch with default behavior may
 * not continue to keep time while the device is asleep. Instead, create one
 * like this:
 *
 * <pre>{@code
...
public final class Stopwatch {
   ...
   *
   * <p><b>Note:</b> the overhead of measurement can be more than a microsecond,
   ...
   * long} to represent elapsed time. Therefore, we recommend using {@link
   * #elapsed()} instead, which returns a strongly-typed {@link Duration}
   * instance.
   *
   ...
   * Duration}. Unlike {@link #elapsed(TimeUnit)}, this method does not lose any
