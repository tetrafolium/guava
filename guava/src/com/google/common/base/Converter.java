 ...
 * {@code converter.reverse().convert(converter.convert(a)).equals(a)} is always
 * true). However, it is very common (perhaps <i>more</i> common) for round-trip
 * conversion to be <i>lossy</i>. Consider an example round-trip using {@link
 * com.google.common.primitives.Doubles#stringConverter}:
 *
 * <ol>
 * <li>{@code stringConverter().convert("1.00")} returns the {@code Double}
 * value {@code 1.0} <li>{@code stringConverter().reverse().convert(1.0)}
 * returns the string {@code "1.0"} -- <i>not</i> the same string ({@code
 * "1.00"}) we started with
 * </ol>
 *
 * <p>Note that it should still be the case that the round-tripped and original
 * objects are <i>similar</i>.
 *
 * <h3>Nullability</h3>
 *
 * <p>A converter always converts {@code null} to {@code null} and non-null
 * references to non-null references. It would not make sense to consider {@code
 * null} and a non-null reference to be "different representations of the same
 * information", since one is distinguishable from <i>missing</i> information
 * and the other is not. The {@link #convert} method handles this null behavior
 * for all converters; implementations of {@link #doForward} and {@link
 * #doBackward} are guaranteed to never be passed {@code null}, and must never
 * return {@code null}.
 *
 *
 ...
 *
 * <ul>
 * <li>Use a provided converter implementation, such as {@link
 * Enums#stringConverter}, {@link
 *     com.google.common.primitives.Ints#stringConverter Ints.stringConverter}
 * or the {@linkplain #reverse reverse} views of these. <li>Convert between
 * specific preset values using {@link
 ...
 * unnecessary (and confusing) to <i>mock</i> the
 *     {@code Converter} type using a mocking framework.
 * <li>Extend this class and implement its {@link #doForward} and {@link
 * #doBackward} methods. <li><b>Java 8 users:</b> you may prefer to pass two
 * lambda expressions or method references to the {@link #from from} factory
 * method.
 * </ul>
 *
 * <p>Using a converter:
 *
 * <ul>
 * <li>Convert one instance in the "forward" direction using {@code
 * converter.convert(a)}. <li>Convert multiple instances "forward" using {@code
 * converter.convertAll(as)}. <li>Convert in the "backward" direction using
 * {@code converter.reverse().convert(b)} or {@code
 *     converter.reverse().convertAll(bs)}.
 * <li>Use {@code converter} or {@code converter.reverse()} anywhere a {@link
 *     java.util.function.Function} is accepted (for example {@link
 * java.util.stream.Stream#map Stream.map}). <li><b>Do not</b> call {@link
 * #doForward} or {@link #doBackward} directly; these exist only to be
 *     overridden.
 * </ul>
 *
 * <h3>Example</h3>
...
public abstract class Converter<A, B> implements Function<A, B> {
   ...
   * {@code a} cannot be converted, an unchecked exception (such as {@link
   ...
   * {@code b} cannot be converted, an unchecked exception (such as {@link
   * IllegalArgumentException}) should be thrown.
   *
   * @param b the instance to convert; will never be null
   * @return the converted instance; <b>must not</b> be null
   * @throws UnsupportedOperationException if backward conversion is not
   *     implemented; this should be very rare. Note that if backward conversion
   *     is not only unimplemented but unimplement<i>able</i> (for example,
   *     consider a {@code Converter<Chicken, ChickenNugget>}), then this is not
   *     logically a {@code Converter} at all, and should just implement {@link
   *     Function}.
   */
  ...
  /**
   * Returns an iterable that applies {@code convert} to each element of {@code
   * fromIterable}. The conversion is done lazily.
   *
   * <p>The returned iterable's iterator supports {@code remove()} if the input
   * iterator does. After a successful {@code remove()} call, {@code
  ...
  /**
   * Returns the reversed view of this converter, which converts {@code
   * this.convert(a)} back to a value roughly equivalent to {@code a}.
   *
   * <p>The returned converter is serializable if {@code this} converter is.
   *
   * <p><b>Note:</b> you should not override this method. It is non-final for
   * legacy reasons.
   */
  @CanIgnoreReturnValue
  ...
  private static final class ReverseConverter<A, B>
     ...
     * backing converter shine through. So, we override the correctedDo*
  ...
  /**
   * Returns a converter whose {@code convert} method applies {@code
   * secondConverter} to the result of this converter. Its {@code reverse}
  ...
  private static final class ConverterComposition<A, B, C>
     ...
     * backing converters shine through (which might even differ from each
     * other!). So, we override the correctedDo* methods, after which the do*
    ...
    public boolean equals(@Nullable Object object) {
      if (object instanceof ConverterComposition) {
        ConverterComposition<?, ?, ?> that =
            (ConverterComposition<?, ?, ?>)object;
   ...
   * {@code true} whenever {@code object} is a
   * {@link Converter} that it considers <i>interchangeable</i> with this one.
   * "Interchangeable" <i>typically</i> means that {@code
   * Objects.equal(this.convert(a), that.convert(a))} is true for all {@code a}
   * of type {@code A} (and similarly for {@code reverse}). Note that a {@code
   * false} result from this method does not imply that the converters are known
  ...
  /**
   * Returns a converter based on separate forward and backward functions. This
   * is useful if the function instances already exist, or so that you can
   * supply lambda expressions. If those circumstances don't apply, you probably
   * don't need to use this; subclass {@code Converter} and implement its {@link
   * #doForward} and {@link #doBackward} methods directly.
   *
   * <p>These functions will never be passed {@code null} and must not under any
  ...
  /**
   * A converter that always converts or reverses an object to itself. Note that
