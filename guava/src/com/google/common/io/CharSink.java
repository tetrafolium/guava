 ...
 * a {@link Writer}, a
 * {@code CharSink} is not an open, stateful stream that can be written to and
 * closed. Instead, it is an immutable <i>supplier</i> of {@code Writer}
 * instances.
 *
 ...
 * </ul>
 *
 * <p>Any {@link ByteSink} may be viewed as a {@code CharSink} with a specific
 * {@linkplain Charset character encoding} using {@link
 * ByteSink#asCharSink(Charset)}. Characters written to the resulting {@code
 * CharSink} will written to the {@code ByteSink} as encoded bytes.
 *
 * @since 14.0
 * @author Colin Decker
 */
...
public abstract class CharSink {
  ...
  /**
   * Opens a new {@link Writer} for writing to this sink. This method returns a
   * new, independent writer each time it is called.
   *
   ...
   * implementations to simply delegate to
   * {@link #openStream()} when the stream returned by that method does not
   * benefit from additional buffering. This method returns a new, independent
   ...
   * Writes the given lines of text to this sink with each line (including the
   * last) terminated with the operating system's default line separator. This
   * method is equivalent to {@code writeLines(lines,
  ...
  private void writeLines(Iterator<? extends CharSequence> lines,
                          String lineSeparator) throws IOException {
  ...
  /**
   * Writes all the text from the given {@link Readable} (such as a {@link
   * Reader}) to this sink. Does not close {@code readable} if it is {@code
   * Closeable}.
   *
