 ...
 *
 * <p>The false positive probability ({@code FPP}) of a Bloom filter is defined
 * as the probability that {@linkplain #mightContain(Object)} will erroneously
 * return {@code true} for an object that has not actually been put in the
 * {@code BloomFilter}.
 *
 * <p>Bloom filters are serializable. They also support a more compact serial
 * representation via the
 * {@link #writeTo} and {@link #readFrom} methods. Both serialized forms will
 * continue to be supported by future versions of this library. However, serial
 * forms generated by newer versions of the code may not be readable by older
 * versions of the code (e.g., a serialized Bloom filter generated today may
 * <i>not</i> be readable by a binary that was compiled 6 months ago).
 *
 * <p>As of Guava 23.0, this class is thread-safe and lock-free. It internally
 * uses atomics and compare-and-swap to ensure correctness when multiple threads
...
public final class BloomFilter<T> implements Predicate<T>, Serializable {
   ...
   * indexes.
   *
   * <p>Implementations should be collections of pure functions (i.e.
   * stateless).
   */
  interface Strategy extends java.io.Serializable {

    /**
     * Sets {@code numHashFunctions} bits of the given bit array, by hashing a
     * user element.
     *
     ...
     * a user element; returns {@code true} if and only if all selected bits are
     * set.
     */
     ...
     * compact serial form. Non-negative values are reserved for enums defined
     * in BloomFilterStrategies; negative values are reserved for any custom,
     * stateful strategy we may define (e.g. any kind of strategy that would
   ...
   * erroneously return
   * {@code true} for an object that has not actually been put in the {@code
   * BloomFilter}.
   *
   * <p>Ideally, this number should be close to the {@code fpp} parameter passed
   * in
   * {@linkplain #create(Funnel, int, double)}, or smaller. If it is
   * significantly higher, it is usually the case that too many elements (more
   * than expected) have been put in the
   ...
   * been added to this Bloom filter. This approximation is reasonably accurate
   * if it does not exceed the value of
  ...
  public long approximateElementCount() {
     ...
     * `bitSize * (1 - (1 - numHashFunctions/bitSize)^n)`. Solving that for n,
     * and approximating `ln x` as `x - 1` when x is close to 1 (why?), gives
   ...
   * instance. Callers must ensure the Bloom filters are appropriately sized to
   ...
   * and yielding a {@link BloomFilter} with false positive probability 3%.
   *
   * <p>Note that if the {@code Collector} receives significantly more elements
   * than specified, the resulting {@code BloomFilter} will suffer a sharp
   * deterioration of its false positive probability.
   *
   * <p>The constructed {@code BloomFilter} will be serializable if the provided
   * {@code Funnel<T>} is.
   *
   * <p>It is recommended that the funnel be implemented as a Java enum. This
   * has the benefit of ensuring proper serialization and deserialization, which
   * is important since {@link #equals} also relies on object identity of
   * funnels.
   *
   * @param funnel the funnel of T's that the constructed {@code BloomFilter}
   *     will use
   * @param expectedInsertions the number of expected insertions to the
   *     constructed {@code BloomFilter}; must be positive
   * @return a {@code Collector} generating a {@code BloomFilter} of the
   *     received elements
   * @since 23.0
   */
  public static <T> Collector<T, ?, BloomFilter<T>>
  ...
  /**
   * Returns a {@code Collector} expecting the specified number of insertions,
   * and yielding a {@link BloomFilter} with the specified expected false
   ...
   *
   * <p>Note that overflowing a {@code BloomFilter} with significantly more
  ...
  static <T> BloomFilter<T> create(Funnel<? super T> funnel,
     ...
     * resulting size is proportional to -log(p), but there is not much of a
     * point after all, e.g. optimalM(1000, 0.0000000000000001) = 76680 which is
  ...
  private Object writeReplace() { return new SerialForm<T>(this); }
   ...
   *
   * <p>Use {@linkplain #readFrom(InputStream, Funnel)} to reconstruct the
   * written BloomFilter.
   */
  ...
  /**
   * Reads a byte stream, which was written by {@linkplain
   ...
   *     its data does not appear to be a BloomFilter serialized using the
  ...
  throws IOException {
    ...
    try {
      ...
      // updated if we add non-stateless strategies (for which we've reserved
      // negative ordinals; see Strategy.ordinal()).
