 ...
 * Future} instances. Note that, whenever possible, it is strongly preferred to
...
public final class JdkFutureAdapters {
   ...
   * is {@linkplain Future#isDone() done}, so each call to this method may claim
   * and hold a thread for an arbitrary length of time. Use of bounded executors
   * or other executors that may fail to execute a task promptly may result in
   * deadlocks.
   *
   * <p>Prefer to create {@code ListenableFuture} instances with {@link
   * SettableFuture}, {@link MoreExecutors#listeningDecorator(
   ...
   * listeners. This implementation will wait on the source future indefinitely,
   * so if the source future never completes, the adapter will never complete
   * either.
   *
  ...
  private static class ListenableFutureAdapter<V>
    ...
    public void addListener(Runnable listener, Executor exec) {
      ...
      // delegate to finish, and when it is done will run the listeners.
        ...
        adapterExecutor.execute(new Runnable() {
          ...
          public void run() {
            try {
              ...
              // Error The task is presumably done, run the listeners.
            }
            executionList.execute();
          }
        });
