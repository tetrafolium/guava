 ...
 *
 * <p>Rate limiters are often used to restrict the rate at which some physical
 * or logical resource is accessed. This is in contrast to {@link
 ...
 * closely related, e.g. see <a
 * href="http://en.wikipedia.org/wiki/Little%27s_law">Little's Law</a>).
 *
 ...
 *
 * <p>It is possible to configure a {@code RateLimiter} to have a warmup period
 ...
 * don't want to submit more than 2 per second: <pre>   {@code final RateLimiter
 * rateLimiter = RateLimiter.create(2.0); // rate is "2 permits per second" void
 * submitTasks(List<Runnable> tasks, Executor executor) { for (Runnable task :
 * tasks) { rateLimiter.acquire(); // may wait executor.execute(task);
 *    }
 *  }}</pre>
 *
 * <p>As another example, imagine that we produce a stream of data, and we want
 * to cap it at 5kb per second. This could be accomplished by requiring a permit
 * per byte, and specifying a rate of 5000 permits per second: <pre>   {@code
 *  final RateLimiter rateLimiter = RateLimiter.create(5000.0); // rate = 5000
 * permits per second void submitPacket(byte[] packet) {
 *    rateLimiter.acquire(packet.length);
 *    networkService.send(packet);
 *  }}</pre>
 *
 * <p>It is important to note that the number of permits requested <i>never</i>
 * affects the throttling of the request itself (an invocation to {@code
 * acquire(1)} and an invocation to
 * {@code acquire(1000)} will result in exactly the same throttling, if any),
 * but it affects the throttling of the <i>next</i> request. I.e., if an
 * expensive task arrives at an idle RateLimiter, it will be granted
 * immediately, but it is the <i>next</i> request that will experience extra
...
// a micro precision would mean a maximum rate of "1MB/s", which might be small
// in some cases.
@ThreadSafe
@Beta
@GwtIncompatible
public abstract class RateLimiter {
   ...
   * limiter is unused, bursts of up to
   * {@code permitsPerSecond} permits will be allowed, with subsequent requests
   ...
   * request after an invocation to
   * {@code setRate} will not be affected by the new rate; it will pay the cost
   * of the previous request, which is in terms of the previous rate.
   *
   * <p>The behavior of the {@code RateLimiter} is not modified in any other
   * way, e.g. if the
   * {@code RateLimiter} was configured with a warmup period of 20 seconds, it
   ...
   * future use, returning the number of microseconds until the reservation can
   * be consumed.
   *
