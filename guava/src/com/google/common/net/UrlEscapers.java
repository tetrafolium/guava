...
public final class UrlEscapers {
   ...
   * safely included in <a href="https://goo.gl/MplK6I">URL form parameter names
   * and values</a>. Escaping is performed with the UTF-8 character encoding.
   * The caller is responsible for <a href="https://goo.gl/9EfkM1">replacing any
   ...
   * through "9" remain the same. <li>The special characters ".", "-", "*", and
   * "_" remain the same. <li>The space character " " is converted into a plus
   * sign "+". <li>All other characters are converted into one or more bytes
   * using UTF-8 encoding and each byte is then represented by the 3-character
   * string "%XY", where "XY" is the two-digit, uppercase, hexadecimal
   * representation of the byte value.
   * </ul>
   *
   * <p>This escaper is suitable for escaping parameter names and values even
   * when <a href="https://goo.gl/utn6M">using the non-standard semicolon</a>,
   ...
   * href="https://url.spec.whatwg.org/#percent-encode">uppercase</a>
  ...
  /**
   * Returns an {@link Escaper} instance that escapes strings so they can be
   * safely included in <a href="https://goo.gl/m2MIf0">URL path segments</a>.
   * The returned escaper escapes all non-ASCII characters, even though <a
   * href="https://goo.gl/e7E0In">many of these are accepted in modern URLs</a>.
   * (<a href="https://goo.gl/jfVxXW">If the escaper were to leave these
   ...
   * <ul>
   * <li>The alphanumeric characters "a" through "z", "A" through "Z" and "0"
   * through "9" remain the same. <li>The unreserved characters ".", "-", "~",
   * and "_" remain the same. <li>The general delimiters "@" and ":" remain the
   * same. <li>The subdelimiters "!", "$", "&amp;", "'", "(", ")", "*", "+",
   * ",", ";", and "=" remain the same. <li>The space character " " is converted
   * into %20. <li>All other characters are converted into one or more bytes
   * using UTF-8 encoding and each byte is then represented by the 3-character
   * string "%XY", where "XY" is the two-digit, uppercase, hexadecimal
   * representation of the byte value.
   * </ul>
  ...
  /**
   * Returns an {@link Escaper} instance that escapes strings so they can be
   * safely included in a <a href="https://goo.gl/xXEq4p">URL fragment</a>. The
   ...
   * same. <li>The subdelimiters "!", "$", "&amp;", "'", "(", ")", "*", "+",
   * ",", ";", and "=" remain the same. <li>The space character " " is converted
   * into %20. <li>Fragments allow unescaped "/" and "?", so they remain the
   * same. <li>All other characters are converted into one or more bytes using
   * UTF-8 encoding and each byte is then represented by the 3-character string
