 ...
 * candidate string {@code "xxx"}, might browsing to
 * {@code "http://xxx/"} result in a webpage being displayed? In the past, this
 * test was frequently done by determining whether the domain ended with a
 * {@linkplain #isPublicSuffix() public suffix} but was not itself a public
 * suffix. However, this test is no longer accurate. There are many domains
 * which are both public suffixes and addressable as hosts; {@code "uk.com"} is
 * one example. As a result, the only useful test to determine if a domain is a
 * plausible web host is
 ...
 * </ol>
 *
 * <p>The normalized values will be returned from {@link #toString()} and {@link
 * #parts()}, and will be reflected in the result of {@link #equals(Object)}.
 *
 * <p><a
 * href="http://en.wikipedia.org/wiki/Internationalized_domain_name">Internationalized
...
public final class InternetDomainName {
  ...
  /**
   * Maximum parts (labels) in a domain name. This value arises from the
   * 255-octet limit described in <a
   * href="http://www.ietf.org/rfc/rfc2181.txt">RFC 2181</a> part 11 with the
   * fact that the encoding of each part occupies at least two bytes (dot plus
   * label externally, length byte plus label internally). Thus, if all labels
   * have the minimum size of one byte, 127 of them will fit.
   */
  private static final int MAX_PARTS = 127;

  /**
   * Maximum length of a full domain name, including separators, and leaving
   * room for the root label. See <a
   ...
   * public suffix according to
   * {@link #isPublicSuffix()} if the domain ends with an excluded domain
   * pattern such as
   * {@code "nhs.uk"}.
   */
   ...
   * Specifically, validation against <a
   * href="http://www.ietf.org/rfc/rfc3490.txt">RFC 3490</a>
   * ("Internationalizing Domain Names in Applications") is skipped, while
   * validation against <a href="http://www.ietf.org/rfc/rfc1035.txt">RFC
   * 1035</a> is relaxed in the following ways: <ul> <li>Any part containing
   * non-ASCII characters is considered valid. <li>Underscores ('_') are
   * permitted wherever dashes ('-') are permitted. <li>Parts other than the
   * final part may start with a digit, as mandated by <a
   ...
   * defined by the Mozilla Foundation's <a
   * href="http://publicsuffix.org/">Public Suffix List</a> (PSL). A public
   * suffix is one under which Internet users can directly register names, such
   * as {@code com},
   * {@code co.uk} or {@code pvt.k12.wy.us}. Examples of domain names that are
   * <i>not</i> public suffixes include {@code google}, {@code google.com} and
   * {@code foo.co.uk}.
   *
   ...
   * {@code www.google.com}, {@code foo.co.uk} and {@code com}, but not for
   * {@code google} or
   * {@code google.foo}. This is the recommended method for determining whether
   * a domain is potentially an addressable host.
   *
   * @since 6.0
   */
   ...
   * returns {@code true} for
   * {@code www.google.com}, {@code foo.co.uk} and {@code bar.ca.us}, but not
   * for {@code google},
   * {@code com}, or {@code
   * google.foo}.
   *
   ...
   * suffixes are also addressable hosts. Use
   ...
   * individual browsers' implementations of cookie controls. See <a
   * href="http://www.ietf.org/rfc/rfc2109.txt">RFC 2109</a> for details.
   *
   * @since 6.0
   */
   ...
   * {@linkplain #isPublicSuffix() public suffix}. For example, returns {@code
   * true} for
   * {@code google.com} and {@code foo.co.uk}, but not for {@code
   * www.google.com} or {@code co.uk}.
   *
   ...
   * addressable hosts. For example, the domain
   * {@code foo.bar.uk.com} has a public suffix of {@code uk.com}, so it would
   * return
   * {@code bar.uk.com} from this method. But {@code uk.com} is itself an
   * addressable host.
   *
   ...
   * InternetDomainName} with the value {@code www.bar.foo.com}. Only lenient
   * validation is performed, as described
  ...
  /**
   * Does the domain name match one of the "wildcard" patterns (e.g. {@code
   * "*.ar"})?
   */
