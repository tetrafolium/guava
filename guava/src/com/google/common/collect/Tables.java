...
public final class Tables {
  ...
  /**
   * Returns a {@link Collector} that accumulates elements into a {@code Table}
   * created using the specified supplier, whose cells are generated by applying
   * the provided mapping functions to the input elements. Cells are inserted
   * into the generated {@code Table} in encounter order.
   *
   * <p>If multiple input elements map to the same row and column, an {@code
   ...
   * into the generated {@code Table} in encounter order.
   *
   * <p>If multiple input elements map to the same row and column, the specified
   * merging function is used to combine the values. Like {@link
   * java.util.stream.Collectors#toMap(java.util.function.Function,
   * java.util.function.Function, BinaryOperator, java.util.function.Supplier)},
   * this Collector throws a {@code NullPointerException} on null values
   * returned from {@code valueFunction}, and treats nulls returned from {@code
   * mergeFunction} as removals of that row/column pair.
   *
   * @since 21.0
   */
  public static <T, R, C, V, I extends Table<R, C, V>> Collector<T, ?, I>
