...
public class ImmutableRangeMap<K extends Comparable<?>, V>
   ...
   * {@code ImmutableRangeMap}. As in {@link Builder}, overlapping ranges are
   * not permitted.
   *
   * @since 23.1
   */
  ...
  copyOf(RangeMap<K, ? extends V> rangeMap) {
    ...
    ImmutableList.Builder<V> valuesBuilder =
        new ImmutableList.Builder<V>(map.size());
  ...
  public static final class Builder<K extends Comparable<?>, V> {
     ...
     *
     * @throws IllegalArgumentException if any two ranges inserted into this
     *     builder overlap
     */
    public ImmutableRangeMap<K, V> build() {
      Collections.sort(entries, Range.<K>rangeLexOrdering().onKeys());
      ImmutableList.Builder<Range<K>> rangesBuilder =
          new ImmutableList.Builder<>(entries.size());
      ImmutableList.Builder<V> valuesBuilder =
          new ImmutableList.Builder<V>(entries.size());
  ...
  public Map.Entry<Range<K>, V> getEntry(K key) {
    ...
    if (index == -1) {
      ...
      Range<K> range = ranges.get(index);
      return range.contains(key) ? Maps.immutableEntry(range, values.get(index))
                                 : null;
    }
  }
