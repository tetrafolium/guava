 ...
 * }</pre>
 *
 * <p>Note that because bounded ranges over {@code int} and {@code long} values
 * are so common, this particular example can be written as just:
 *
 * <pre>{@code
 * ContiguousSet.closed(5, 42)
 * }</pre>
 *
 * <p><b>Warning:</b> Be extremely careful what you do with conceptually large
 * instances (such as
 * {@code ContiguousSet.create(Range.greaterThan(0),
 * DiscreteDomain.integers()}). Certain operations on such a set can be
...
public abstract class ContiguousSet<C extends Comparable>
   ...
   *
   * @throws IllegalArgumentException if neither range nor the domain has a
   *     lower bound, or if neither has an upper bound
   *
   * @since 13.0
   */
  public static <C extends Comparable> ContiguousSet<C>
  create(Range<C> range, DiscreteDomain<C> domain) {
    ...
    try {
      if (!range.hasLowerBound()) {
        effectiveRange =
            effectiveRange.intersection(Range.atLeast(domain.minValue()));
      }
      if (!range.hasUpperBound()) {
        effectiveRange =
            effectiveRange.intersection(Range.atMost(domain.maxValue()));
      }
   ...
   * {@code lower} (inclusive) to {@code upper} (inclusive). (These are the same
   * values contained in {@code Range.closed(lower, upper)}.)
   *
  ...
  /**
   * Returns a contiguous set containing all {@code int} values from {@code
   * lower} (inclusive) to
   * {@code upper} (exclusive). If the endpoints are equal, an empty set is
   * returned. (These are the same values contained in {@code
  ...
  /**
   * Returns a contiguous set containing all {@code long} values from {@code
   * lower} (inclusive) to
   * {@code upper} (exclusive). If the endpoints are equal, an empty set is
   * returned. (These are the same values contained in {@code
   * Range.closedOpen(lower, upper)}.)
  ...
  /**
   * Returns the set of values that are contained in both this set and the
   * other.
   *
   ...
   * maximum values contained in this set.  This is equivalent to {@code
   * range(CLOSED, CLOSED)}.
   *
  ...
  /**
   * Returns the minimal range with the given boundary types for which all
   * values in this set are
   * {@linkplain Range#contains(Comparable) contained} within the range.
   *
   * <p>Note that this method will return ranges with unbounded endpoints if
   * {@link BoundType#OPEN} is requested for a domain minimum or maximum.  For
   * example, if {@code set} was created from the range {@code
   * [1..Integer.MAX_VALUE]} then {@code set.range(CLOSED, OPEN)} must return
   * {@code [1..∞)}.
   *
   * @throws NoSuchElementException if this set is empty
   */
  ...
  /**
   * Returns a short-hand representation of the contents such as {@code
   * "[1..100]"}.
   */
  @Override
  ...
  /**
   * Not supported. {@code ContiguousSet} instances are constructed with {@link
   * #create}. This method exists only to hide {@link ImmutableSet#builder} from
   * consumers of {@code ContiguousSet}.
   *
