 ...
 *   // BAD CODE! The returned multiset is actually an unsorted
 * ImmutableMultiset!}</pre>
 *
 * <p>While we could put the overrides in {@link ImmutableSortedMultiset}
 * itself, it seems clearer to separate these "do not call" methods from those
...
abstract class ImmutableSortedMultisetFauxverideShim<E>
   ...
   * exists only to hide {@link ImmutableMultiset#toImmutableMultiset} from
  ...
  /**
   * Not supported. Use {@link ImmutableSortedMultiset#naturalOrder}, which
   * offers better type-safety, instead. This method exists only to hide {@link
   * ImmutableMultiset#builder} from consumers of {@code
   ...
   * a non-{@code Comparable} element.</b> Proper calls will resolve to the
   * version in {@code ImmutableSortedMultiset}, not this dummy version.
   *
   * @throws UnsupportedOperationException always
   ...
   * @deprecated <b>Pass the parameters of type {@code Comparable} to use {@link
   *             ImmutableSortedMultiset#of(Comparable, Comparable, Comparable,
   * Comparable, Comparable, Comparable, Comparable...)} . </b>
   */
  @Deprecated
   ...
   * providing only the properly typed "<E extends Comparable<E>>
   * copyOf(Iterable<E>)" in ImmutableSortedMultiset (and likewise for the
   * Iterator equivalent). However, due to a change in Sun's interpretation of
   * the JLS (as described at http://bugs.sun.com/view_bug.do?bug_id=6182950),
   * the OpenJDK 7 compiler available as of this writing rejects our attempts.
   * To maintain compatibility with that version and with any other compilers
   * that interpret the JLS similarly, there is no definition of copyOf() here,
   * and the definition in ImmutableSortedMultiset matches that in
   * ImmutableMultiset.
   *
