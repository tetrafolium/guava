...
final class SortedLists {
  ...
  enum KeyPresentBehavior {
     ...
     * No guarantees are made as to which index is returned, if more than one
     * element compares as equal to the key.
     */
    ANY_PRESENT {
      @Override
    ...
    /**
     * Return the index of the last list element that compares as equal to the
     * key.
     */
    LAST_PRESENT {
      @Override
    ...
    /**
     * Return the index of the first list element that compares as equal to the
     * key.
     */
    FIRST_PRESENT {
      @Override
      <E> int resultIndex(Comparator<? super E> comparator, E key,
        ...
        // Of course, we have to use binary search to find the precise
        // breakpoint... Everything between lower and upper inclusive compares
        // at <= 0.
        while (lower < upper) {
    ...
    /**
     * Return the index of the first list element that compares as greater than
     * the key, or {@code list.size()} if there is no such element.
     */
    FIRST_AFTER {
      @Override
    ...
    /**
     * Return the index of the last list element that compares as less than the
     * key, or {@code -1} if there is no such element.
     */
    LAST_BEFORE {
      @Override
      public <E> int resultIndex(Comparator<? super E> comparator, E key,
  ...
  enum KeyAbsentBehavior {
    ...
    /**
     * Return the index of the next higher element in the list, or {@code
    ...
    /**
     * Return {@code ~insertionIndex}, where {@code insertionIndex} is defined
     * as the point at which the key would be inserted into the list: the index
     * of the next higher element in the list, or {@code list.size()} if there
     * is no such element.
     *
     * <p>Note that the return value will be {@code >= 0} if and only if there
   ...
   *
   * <p>Equivalent to {@link #binarySearch(List, Function, Object, Comparator,
   * KeyPresentBehavior, KeyAbsentBehavior)} using {@link Ordering#natural}.
   */
   ...
   *
   * <p>Equivalent to
   * {@link #binarySearch(List, Object, Comparator, KeyPresentBehavior,
   ...
   * the specified comparator (as by the
   ...
   *
   * <p>If there are elements in the list which compare as equal to the key, the
   * choice of
   * {@link KeyPresentBehavior} decides which index is returned. If no elements
   * compare as equal to the key, the choice of {@link KeyAbsentBehavior}
   ...
   * @param absentBehavior the specification for what to do if no elements of
   *     the list compare as
   *        equal to the key.
   * @return the index determined by the {@code KeyPresentBehavior}, if the key
   *     is in the list;
