...
public class ImmutableSetMultimap<K, V>
   ...
   * ImmutableSetMultimap} whose keys and values are the result of applying the
   * provided mapping functions to the input elements.
   *
   * <p>For streams with {@linkplain java.util.stream#Ordering defined encounter
   * order}, that order is preserved, but entries are <a
   ...
   *           .collect(toImmutableSetMultimap(str -> str.charAt(0), str ->
   * str.substring(1)));
   *
   *   // is equivalent to
   *
   ...
   */
  @Beta
  public static <T, K, V> Collector<T, ?, ImmutableSetMultimap<K, V>>
  ...
  /**
   * Returns a {@code Collector} accumulating entries into an {@code
   * ImmutableSetMultimap}. Each input element is mapped to a key and a stream
   * of values, each of which are put into the resulting {@code Multimap}, in
   * the encounter order of the stream and the encounter order of the streams of
   * values.
   *
   * <p>Example:
   *
   * <pre>{@code
   * static final ImmutableSetMultimap<Character, Character>
   * FIRST_LETTER_MULTIMAP = Stream.of("banana", "apple", "carrot", "asparagus",
   * "cherry") .collect( flatteningToImmutableSetMultimap( str -> str.charAt(0),
   *                  str -> str.substring(1).chars().mapToObj(c -> (char) c));
   *
   * // is equivalent to
   *
   * static final ImmutableSetMultimap<Character, Character>
   * FIRST_LETTER_MULTIMAP = ImmutableSetMultimap.<Character,
   * Character>builder() .putAll('b', Arrays.asList('a', 'n', 'a', 'n', 'a'))
   ...
   * static final ImmutableSetMultimap<Character, Character>
   * FIRST_LETTER_MULTIMAP = ImmutableSetMultimap.<Character,
   * Character>builder() .putAll('b', Arrays.asList('a', 'n')) .putAll('a',
   * Arrays.asList('p', 'l', 'e', 's', 'a', 'r', 'g', 'u')) .putAll('c',
   * Arrays.asList('a', 'r', 'o', 't', 'h', 'e', 'y')) .build();
   * }
   * }</pre>
   *
   * @since 21.0
  ...
  flatteningToImmutableSetMultimap(
    ...
    return Collectors.collectingAndThen(
      Multimaps.flatteningToMultimap(
        input
        ->checkNotNull(keyFunction.apply(input)),
        input
        ->valuesFunction.apply(input).peek(Preconditions::checkNotNull),
        MultimapBuilder.linkedHashKeys()
        .linkedHashSetValues() ::<K, V>build),
  ...
  public static final class Builder<K, V>
    ...
    public Builder<K, V> put(Entry<? extends K, ? extends V> entry) {
      builderMultimap.put(checkNotNull(entry.getKey()),
    ...
    public ImmutableSetMultimap<K, V> build() {
      if (keyComparator != null) {
        Multimap<K, V> sortedCopy = MultimapBuilder.linkedHashKeys()
            .linkedHashSetValues()
            .<K, V>build();
  ...
  private static final class EntrySet<K, V> extends ImmutableSet<Entry<K, V>> {
    ...
    EntrySet(ImmutableSetMultimap<K, V> multimap) { this.multimap = multimap; }
  ...
  valueSet(@Nullable Comparator<? super V> valueComparator,
  ...
  private static <V> ImmutableSet<V>
  emptySet(@Nullable Comparator<? super V> valueComparator) {
    return (valueComparator == null)
        ? ImmutableSet.<V>of()
  ...
  valuesBuilder(@Nullable Comparator<? super V> valueComparator) {
    return (valueComparator == null)
  ...
  throws IOException, ClassNotFoundException {
    ...
    for (int i = 0; i < keyCount; i++) {
      ...
      ImmutableSet.Builder<Object> valuesBuilder =
          valuesBuilder(valueComparator);
    ...
    FieldSettersHolder.EMPTY_SET_FIELD_SETTER.set(this,
        emptySet(valueComparator));
  }
