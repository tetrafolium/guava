 ...
 * change. Instances of this class may be obtained with {@link
 * #copyOf(Network)}.
 *
...
public final class ImmutableNetwork<N, E> extends ConfigurableNetwork<N, E> {
   ...
   */
  @Deprecated
  public static <N, E> ImmutableNetwork<N, E>
  ...
  getNodeConnections(Network<N, E> network) {
    // ImmutableMap.Builder maintains the order of the elements as inserted, so
    // the map will have whatever ordering the network's nodes do, so
    // ImmutableSortedMap is unnecessary even if the input nodes are sorted.
    ImmutableMap.Builder<N, NetworkConnections<N, E>> nodeConnections =
  ...
  private static <N, E> Map<E, N>
  getEdgeToReferenceNode(Network<N, E> network) {
    // ImmutableMap.Builder maintains the order of the elements as inserted, so
    // the map will have whatever ordering the network's edges do, so
    // ImmutableSortedMap is unnecessary even if the input edges are sorted.
  ...
  connectionsOf(Network<N, E> network, N node) {
    if (network.isDirected()) {
      Map<E, N> inEdgeMap =
          Maps.asMap(network.inEdges(node), sourceNodeFn(network));
      Map<E, N> outEdgeMap =
          Maps.asMap(network.outEdges(node), targetNodeFn(network));
