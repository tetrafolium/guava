 ...
 *
 * <p>This class maintains a map of nodes to {@link NetworkConnections}. This
 * class also maintains a map of edges to reference nodes. The reference node is
 * defined to be the edge's source node on directed graphs, and an arbitrary
 * endpoint of the edge on undirected graphs.
 *
 * <p>Collection-returning accessors return unmodifiable views: the view
 * returned will reflect changes to the graph (if the graph is mutable) but may
 * not be modified by the user.
 *
 * <p>The time complexity of all collection-returning accessors is O(1), since
 * views are returned.
 *
 * @author James Sexton
...
class ConfigurableNetwork<N, E> extends AbstractNetwork<N, E> {
  ...
  ConfigurableNetwork(NetworkBuilder<? super N, ? super E> builder,
    ...
    // This optimizes methods that access the same node(s) repeatedly, such as
