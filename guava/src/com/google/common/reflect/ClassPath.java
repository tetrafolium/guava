 ...
 *
 * <ul>
 *   <li>Looks only for files and JARs in URLs available from {@link
 * URLClassLoader} instances or the {@linkplain
 * ClassLoader#getSystemClassLoader() system class loader}. <li>Only understands
 * {@code file:} URLs.
 * </ul>
 *
 * <p>In the case of directory classloaders, symlinks are supported but cycles
 * are not traversed. This guarantees discovery of each <em>unique</em> loadable
...
public final class ClassPath {
   ...
   * from {@code classloader} and its ancestor class loaders.
   *
   * <p><b>Warning:</b> {@code ClassPath} can find classes and resources only
   * from:
   *
   * <ul>
   *   <li>{@link URLClassLoader} instances' {@code file:} URLs
   *   <li>the {@linkplain ClassLoader#getSystemClassLoader() system class
   * loader}. To search the system class loader even when it is not a {@link
   * URLClassLoader} (as in Java 9), {@code ClassPath} searches the files from
   * the {@code java.class.path} system property.
   * </ul>
   *
   * @throws IOException if the attempt to read class path resources (jar files
   *     or directories) failed.
   */
  ...
  public static class ResourceInfo {
     ...
     *
     * @throws NoSuchElementException if the resource cannot be loaded through
     *     the class loader, despite physically existing in the class path.
     */
    ...
    /**
     * Returns a {@link ByteSource} view of the resource from which its bytes
     * can be read.
     *
     * @throws NoSuchElementException if the resource cannot be loaded through
     *     the class loader, despite physically existing in the class path.
    ...
    /**
     * Returns a {@link CharSource} view of the resource from which its bytes
     * can be read as characters decoded with the given {@code charset}.
     *
     * @throws NoSuchElementException if the resource cannot be loaded through
     *     the class loader, despite physically existing in the class path.
     * @since 20.0
  ...
  public static final class ClassInfo extends ResourceInfo {
     ...
     * class.
     *
     * <p>Behaves identically to {@link Package#getName()} but does not require
     ...
     * code.
     *
     * <p>Behaves identically to {@link Class#getSimpleName()} but does not
    ...
    public String getSimpleName() {
      ...
      if (lastDollarSign != -1) {
        ...
        // anonymous classes are entirely numeric whereas local classes have the
        // user supplied name as a suffix
      ...
      // Since this is a top level class, its simple name is always the part
      // after package name.
     ...
     *
     * <p>Behaves identically to {@link Class#getName()} but does not require
     ...
     *     class depends on. For example, {@link NoClassDefFoundError}.
     */
    public Class<?> load() {
      try {
  ...
  /**
   * Abstract class that scans through the class path represented by a {@link
   * ClassLoader} and calls
   * {@link #scanDirectory} and {@link #scanJarFile} for directories and jar
   * files on the class path respectively.
   */
  abstract static class Scanner {
     ...
     */
    @VisibleForTesting
    static ImmutableSet<File>
    getClassPathFromManifest(File jarFile, @Nullable Manifest manifest) {
      if (manifest == null) {
    ...
    @VisibleForTesting
    static ImmutableMap<File, ClassLoader>
    ...
    private static ImmutableList<URL>
    ...
    /**
     * Returns the URLs in the class path specified by the {@code
     * java.class.path} {@linkplain System#getProperty system property}.
     */
    ...
    static ImmutableList<URL> parseJavaClassPath() {
      ...
      for (String entry :
        ...
        try {
          ...
          } catch (SecurityException e) { // File.toURI checks to see if the
                                          // file is a directory
  ...
  static final class DefaultScanner extends Scanner {
     ...
     * encountered. Symlinks which have already been traversed in the current
     * tree path will be skipped to eliminate cycles; otherwise symlinks are
     * traversed.
     *
     * @param directory the root of the directory to scan
     * @param classloader the classloader that includes resources found in
     *     {@code directory}
     * @param packagePrefix resource path prefix inside {@code classloader} for
     *     any files found under {@code directory}
     * @param currentPath canonical files already visited in the current
