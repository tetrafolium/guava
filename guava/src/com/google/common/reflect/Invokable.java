 ...
 * TypeToken#constructor} will resolve the type parameters of the method or
 * constructor in the context of the owner type, which may be a subtype of the
 * declaring class. For example:
 *
 * <pre>   {@code
 *   Method getMethod = List.class.getMethod("get", int.class);
 *   Invokable<List<String>, ?> invokable = new TypeToken<List<String>>()
 * {}.method(getMethod); assertEquals(TypeToken.of(String.class),
 * invokable.getReturnType()); // Not Object.class! assertEquals(new
 * TypeToken<List<String>>() {}, invokable.getOwnerType());}</pre>
 *
...
public abstract class Invokable<T, R>
   ...
   * private, static or final methods, or methods declared by final classes are
   * not overridable.
   */
   ...
   *     Java language access control and the underlying method or constructor
   *     is inaccessible.
  ...
  /**
   * Returns all declared parameters of this {@code Invokable}. Note that if
   * this is a constructor of a non-static inner class, unlike {@link
   * Constructor#getParameterTypes}, the hidden
   * {@code this} parameter of the enclosing class is excluded from the returned
   * parameters.
   */
  ...
  public final ImmutableList<TypeToken<? extends Throwable>>
  getExceptionTypes() {
    ImmutableList.Builder<TypeToken<? extends Throwable>> builder =
        ImmutableList.builder();
    for (Type type : getGenericExceptionTypes()) {
      // getGenericExceptionTypes() will never return a type that's not
      // exception
      @SuppressWarnings("unchecked")
  ...
  /**
   * Explicitly specifies the return type of this {@code Invokable}. For
   * example: <pre>   {@code Method factoryMethod =
   * Person.class.getMethod("create"); Invokable<?, Person> factory =
   * Invokable.of(getNameMethod).returning(Person.class);}</pre>
   */
  ...
  public final <R1 extends R> Invokable<T, R1>
  ...
  static class ConstructorInvokable<T> extends Invokable<T, T> {
    ...
    /**
     * If the class is parameterized, such as {@link java.util.ArrayList
     ...
     * prepended before those of the constructor's. This is an arbitrary rule
     * since no existing language spec mandates one way or the other. From the
     * declaration syntax, the class type parameter appears first, but the call
     * syntax may show up in opposite order such as {@code new <A>Foo<B>()}.
     */
    @Override
    public final TypeVariable<?>[] getTypeParameters() {
      TypeVariable<?>[] declaredByClass =
    ...
    private boolean mayNeedHiddenThis() {
      ...
      if (enclosingMethod != null) {
        ...
        // Strictly, this doesn't necessarily indicate a hidden 'this' in the
        // case of static initializer. But there seems no way to tell in that
        // case. :( This may cause issues when an anonymous class is created
        // inside a static initializer, and the class's constructor's first
        // parameter happens to be the enclosing class. In such case, we may
