...
public final class PairedStatsAccumulator {
  ...
  // well as those of the stat methods of this class.
  ...
  public void add(double x, double y) {
    // We extend the recursive expression for the one-variable case at Art of
    // Computer Programming vol. 2, Knuth, 4.2.2, (16) to the two-variable case.
    // We have two value series x_i and y_i. We define the arithmetic means X_n
    // = 1/n \sum_{i=1}^n x_i, and Y_n = 1/n \sum_{i=1}^n y_i. We also define
    // the sum of the products of the differences from the means
    ...
    //               = x_n y_n - X_n [ y_n + (n-1) Y_{n-1} ] + [ n X_n - x_n ]
    //               Y_{n-1} = x_n y_n - X_n y_n - x_n Y_{n-1} + X_n Y_{n-1} =
    //               (x_n - X_n) (y_n - Y_{n-1})
    xStats.add(x);
   ...
   * alone.
   */
  public Stats yStats() { return yStats.snapshot(); }
   ...
   *
   * @throws IllegalStateException if the dataset is empty or contains a single
   *     pair of values, or both the {@code x} and {@code y} dataset have zero
   *     population variance
   */
