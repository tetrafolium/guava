...
/**
 * A class for arithmetic on doubles that is not covered by {@link
 * java.lang.Math}.
 *
...
public final class DoubleMath {
   ...
   *     <li>{@code x}, after being rounded to a mathematical integer using the
   * specified rounding mode, is either less than {@code Integer.MIN_VALUE} or
   * greater than {@code Integer.MAX_VALUE} <li>{@code x} is not a mathematical
   * integer and {@code mode} is
   ...
   *     <li>{@code x} is infinite or NaN
   *     <li>{@code x}, after being rounded to a mathematical integer using the
   * specified rounding mode, is either less than {@code Long.MIN_VALUE} or
   * greater than {@code Long.MAX_VALUE} <li>{@code x} is not a mathematical
   * integer and {@code mode} is
   *         {@link RoundingMode#UNNECESSARY}
   *     </ul>
   */
  ...
  public static BigInteger roundToBigInteger(double x, RoundingMode mode) {
    ...
    BigInteger result =
        BigInteger.valueOf(significand).shiftLeft(exponent - SIGNIFICAND_BITS);
   ...
   *
   * <p>If the result of this method will be immediately rounded to an {@code
   * int},
   ...
   * {@code int}.
   *
   * <p>Regardless of the rounding mode, this is faster than {@code (int)
   * log2(x)}.
   *
  ...
  public static int log2(double x, RoundingMode mode) {
    ...
    case HALF_UP:
      ...
      // result," so log2(x) is never exactly exponent + 0.5.
   ...
   *
   * <p>This is equivalent to, but not necessarily implemented as, the
   * expression {@code !Double.isNaN(x) && !Double.isInfinite(x) && x ==
   * Math.rint(x)}.
   */
  @GwtIncompatible
  // java.lang.Math.getExponent, com.google.common.math.DoubleUtils
  ...
  public static double factorial(int n) {
    ...
    if (n > MAX_FACTORIAL) {
      ...
      // Multiplying the last (n & 0xf) values into their own accumulator gives
      // a more accurate result than multiplying by everySixteenthFactorial[n >>
      // 4] directly.
      double accum = 1.0;
  ...
  @VisibleForTesting
  static final double[] everySixteenthFactorial = {0x1.0p0,
                                                   0x1.30777758p44,
  ...
  /**
   * Returns {@code true} if {@code a} and {@code b} are within {@code
   * tolerance} of each other.
   *
   * <p>Technically speaking, this is equivalent to {@code Math.abs(a - b) <=
   * tolerance || Double.valueOf(a).equals(Double.valueOf(b))}.
   *
   ...
   * equal. <li>Positive and negative zero are always fuzzily equal. <li>If
   * {@code tolerance} is zero, and neither {@code a} nor {@code b} is NaN, then
   ...
   * are fuzzily equal. <li>With finite tolerance, {@code
   * Double.POSITIVE_INFINITY} and {@code Double.NEGATIVE_INFINITY} are fuzzily
   * equal only to themselves.
   * </ul>
   *
   * <p>This is reflexive and symmetric, but <em>not</em> transitive, so it is
   ...
   * particular, like
   * {@link Double#compare(double, double)}, it treats all NaN values as equal
   * and greater than all other values (including {@link
   * Double#POSITIVE_INFINITY}).
   *
   * <p>This is <em>not</em> a total ordering and is <em>not</em> suitable for
   * use in
   ...
   *     non-finite value
   * @deprecated Use {@link Stats#meanOf} instead, noting the less strict
  ...
  public static double mean(int... values) {
    ...
    // values mean that, in this case only, we can compute the sum as a long
    // without risking overflow or loss of precision. So we do that, as it's
    // slightly quicker than the Knuth algorithm.
    long sum = 0;
   ...
   *     {@code double} values (this may cause loss of precision for longs of
