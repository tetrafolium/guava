 ...
 * counterparts.
 *
 * <p>The implementations of many methods in this class are based on material
 * from Henry S. Warren, Jr.'s <i>Hacker's Delight</i>, (Addison Wesley, 2002).
 *
 * <p>Similar functionality for {@code int} and for {@link BigInteger} can be
 * found in
 * {@link IntMath} and {@link BigIntegerMath} respectively. For other common
 * operations on
...
public final class LongMath {
   ...
   * @throws ArithmeticException of the next-higher power of two is not
   *     representable as a
  ...
  public static long floorPowerOfTwo(long x) {
    ...
    // Long.highestOneBit was buggy on GWT.  We've fixed it, but I'm not certain
  ...
  /**
   * Returns 1 if {@code x < y} as unsigned longs, and 0 otherwise. Assumes that
   * x - y fits into a signed long. The implementation is branch-free, and
   ...
   *
   * @throws IllegalArgumentException if {@code x <= 0}
   * @throws ArithmeticException if {@code mode} is {@link
   *     RoundingMode#UNNECESSARY} and {@code x} is not a power of two
   */
   ...
   * @throws IllegalArgumentException if {@code x <= 0}
   * @throws ArithmeticException if {@code mode} is {@link
   *     RoundingMode#UNNECESSARY} and {@code x} is not a power of ten
   */
  ...
  static int log10Floor(long x) {
     ...
     * floor(log2(x))), we can narrow the possible floor(log10(x)) values to
     * two. For example, if floor(log2(x)) is 6, then 64 <= x < 128, so
     * floor(log10(x)) is either 1 or 2.
     */
  ...
  @GwtIncompatible // TODO
  @VisibleForTesting
  static final long[] powersOf10 = {1L,
                                    10L,
                                    100L,
                                    1000L,
                                    10000L,
  ...
  @GwtIncompatible // TODO
  @VisibleForTesting
  static final long[] halfPowersOf10 = {3L,
                                        31L,
                                        316L,
                                        3162L,
                                        31622L,
   ...
   * it will be equal to
   * {@code BigInteger.valueOf(b).pow(k).longValue()}. This implementation runs
   * in {@code O(log k)} time.
   *
  ...
  public static long sqrt(long x, RoundingMode mode) {
     ...
     * (long) Math.sqrt(k * k) <= (long) Math.sqrt(x) <= (long) Math.sqrt((k +
     * 1) * (k + 1)) since casting to long is monotonic k <= (long) Math.sqrt(x)
     * <= k + 1 since (long) Math.sqrt(k * k) == k, as checked exhaustively in
   ...
   *     UNNECESSARY} and {@code a} is not an integer multiple of {@code b}
   */
  @GwtIncompatible // TODO
  @SuppressWarnings("fallthrough")
  ...
  public static long gcd(long a, long b) {
     ...
     * what do you return on gcd(0, Long.MIN_VALUE)? BigInteger.gcd would return
     * positive 2^63, but positive 2^63 isn't an int.
     */
     ...
     * http://en.wikipedia.org/wiki/Binary_GCD_algorithm. This is >60% faster
   ...
   *
   * @throws ArithmeticException if {@code a + b} overflows in signed {@code
   *     long} arithmetic
   */
  @GwtIncompatible // TODO
  ...
  public static long checkedMultiply(long a, long b) {
     ...
     * it's definitely bad. We do the leadingZeros check to avoid the division
     * below if at all possible.
     *
     * Otherwise, if b == Long.MIN_VALUE, then the only allowed values of a are
     * 0 and 1. We take care of all a < 0 with their own check, because in
     * particular, the case a == -1 will incorrectly pass the division check
     * below.
     *
  ...
  /**
   * Returns the difference of {@code a} and {@code b} unless it would overflow
   * or underflow in which case {@code Long.MAX_VALUE} or {@code Long.MIN_VALUE}
  ...
  public static long saturatedMultiply(long a, long b) {
    ...
    // the return value if we will overflow (which we calculate by overflowing a
    // long :) )
  ...
  public static long saturatedPow(long b, int k) {
    ...
    long accum = 1;
    // if b is negative and k is odd then the limit is MIN otherwise the limit
    // is MAX
   ...
   * integers, {@code 1} if
   * {@code n == 0}, or {@link Long#MAX_VALUE} if the result does not fit in a
   * {@code long}.
   *
   ...
   * of {@code n} and
   * {@code k}, or {@link Long#MAX_VALUE} if the result does not fit in a {@code
   * long}.
   *
   * @throws IllegalArgumentException if {@code n < 0}, {@code k < 0}, or {@code
   *     k > n}
   */
  public static long binomial(int n, int k) {
    ...
    default:
      if (n < factorials.length) {
         ...
         * overflow. We adapt the technique previously used by BigIntegerMath:
         * maintain separate numerator and denominator accumulators, multiplying
         * the fraction into result when near overflow.
         */
        for (int i = 2; i <= k; i++, n--) {
  ...
  static long multiplyFraction(long x, long numerator, long denominator) {
    ...
    // exact, so denominator must be a divisor of numerator.
   ...
   */
  static final int[] biggestBinomials = {Integer.MAX_VALUE,
                                         Integer.MAX_VALUE,
                                         Integer.MAX_VALUE,
                                         3810779,
                                         121977,
   ...
   */
  @VisibleForTesting
  static final int[] biggestSimpleBinomials = {Integer.MAX_VALUE,
                                               Integer.MAX_VALUE,
                                               Integer.MAX_VALUE,
                                               2642246,
                                               86251,
  ...
  // multiply/divide algorithm would lead to an overflow.

  static boolean fitsInInt(long x) { return (int)x == x; }
  ...
  /*
   * This bitmask is used as an optimization for cheaply testing for divisiblity
   * by 2, 3, or 5. Each bit is set to 1 for all remainders that indicate
   * divisibility by 2, 3, or 5, so 1, 7, 11, 13, 17, 19, 23, 29 are set to 0.
   * 30 and up don't matter because they won't be hit.
   */
   ...
   * of <i>smaller</i> positive integers. Returns {@code false} if {@code n} is
   * zero, one, or a composite number (one which <i>can</i> be factored into
  ...
  private enum MillerRabinTester {
    ...
    LARGE {
      /**
       * Returns (a + b) mod m. Precondition: {@code 0 <= a}, {@code b < m <
       * 2^63}.
       */
      ...
      private long times2ToThe32Mod(long a, long m) {
        ...
        do {
          int shift =
              Math.min(remainingPowersOf2, Long.numberOfLeadingZeros(a));
          // shift is either the number of powers of 2 left to multiply a by, or
          // the biggest shift possible while keeping a in an unsigned long.
      ...
      long mulMod(long a, long b, long m) {
        ...
        /*
         * a * b == aHi * bHi * 2^64 + (aHi * bLo + aLo * bHi) * 2^32 + aLo *
         * bLo.
         *       == (aHi * bHi * 2^32 + aHi * bLo + aLo * bHi) * 2^32 + aLo *
         * bLo
         *
    ...
    static boolean test(long base, long n) {
      ...
      // FLOOR_SQRT_MAX_LONG, so long as n <= FLOOR_SQRT_MAX_LONG.
    ...
    /**
     * Returns true if n is a strong probable prime relative to the specified
     * base.
     */
