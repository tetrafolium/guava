...
class LocalCache<K, V>
   ...
   * either of the constructors with arguments. MUST be a power of two {@code <=
  ...
  LocalCache(CacheBuilder<? super K, ? super V> builder,
    ...
    // unless maximumSize/Weight is specified in which case ensure that each
    // segment gets at least 10 entries. The special casing for size-based
    // eviction is only necessary because that eviction happens per segment
  ...
  interface ValueReference<K, V> {
    ...
    /**
     * Waits for a value that may still be loading. Unlike get(), this method
     * can block (in the case of FutureValueReference).
     *
    ...
    /**
     * Returns the weight of this entry. This is assumed to be static between
     * calls to setValue.
     */
    int getWeight();

    /**
     * Returns the entry associated with this value reference, or {@code null}
    ...
    /**
     * Returns true if a new value is currently loading, regardless of whether
     * or not there is an existing value. It is assumed that the return value of
     * this method is constant for any given ValueReference instance.
     */
    boolean isLoading();

    /**
     * Returns true if this reference contains an active value, meaning one that
     * is still considered present in the cache. Active values consist of live
     * values, which are returned by cache lookups, and dead values, which have
     * been evicted but awaiting removal. Non-active values consist strictly of
     * loading values, though during refresh a value may be both active and
     * loading.
     */
    boolean isActive();
  }
  ...
  interface ReferenceEntry<K, V> {
     ...
     * doubly-linked list. New entries are added at the tail of the list at
     * write time; stale entries are expired from the head of the list.
     */
    ...
    /*
     * Implemented by entries that use write order. Write entries are maintained
  ...
  private enum NullEntry implements ReferenceEntry<Object, Object> {
    ...
    @Override
    public void
    setValueReference(ValueReference<Object, Object> valueReference) {}
    ...
    @Override
    public void
    setPreviousInAccessQueue(ReferenceEntry<Object, Object> previous) {}
    ...
    @Override
    public void
    setPreviousInWriteQueue(ReferenceEntry<Object, Object> previous) {}
  }
  ...
  static class StrongValueReference<K, V> implements ValueReference<K, V> {
    ...
    StrongValueReference(V referent) { this.referent = referent; }
   ...
   * partially-collected, loading, or expired. Unlike {@link
   * Segment#getLiveValue} this method does not attempt to cleanup stale
   * entries. As such it should only be called outside of a segment context,
  ...
  static class Segment<K, V> extends ReentrantLock {
     ...
     * modCounts change during a traversal of segments loading size or checking
     ...
     * updating the access list's ordering. It is drained as a batch operation
     * when either the DRAIN_THRESHOLD is crossed or a write occurs on the
     * segment.
     */
    ...
    /**
     * A queue of elements currently in the map, ordered by write time. Elements
     * are added to the tail of the queue on write.
     */
    @GuardedBy("this") final Queue<ReferenceEntry<K, V>> writeQueue;

    /**
     * A queue of elements currently in the map, ordered by access time.
     * Elements are added to the tail of the queue on access (note that writes
     * count as accesses).
     */
    ...
    Segment(LocalCache<K, V> map, int initialCapacity, long maxSegmentWeight,
      ...
      keyReferenceQueue =
          map.usesKeyReferences() ? new ReferenceQueue<K>() : null;

      valueReferenceQueue =
          map.usesValueReferences() ? new ReferenceQueue<V>() : null;
     ...
     * Returns the new entry, or {@code null} if {@code original} was already
     * garbage collected.
     */
    @GuardedBy("this")
    ReferenceEntry<K, V> copyEntry(ReferenceEntry<K, V> original,
    ...
    /**
     * Sets a new value of an entry. Adds newly created entries at the end of
     * the access queue.
     */
    @GuardedBy("this")
    ...
    V lockedGetOrLoad(K key, int hash, CacheLoader<? super K, V> loader)
      ...
      if (createNewEntry) {
        try {
          ...
          // load is detected. This may be circumvented when an entry is copied,
          // but will fail fast most of the time.
          synchronized (e) {
    ...
    V compute(K key, int hash,
      ...
      try {
        ...
        // loading value if the value for the key is already being computed.
     ...
     * {@code key} if it was refreshed inline, or
    ...
    /**
     * Returns a newly inserted {@code LoadingValueReference}, or null if the
     * live value reference is already loading.
     */
    @Nullable
        ...
        for (e = first; e != null; e = e.getNext()) {
          ...
          if (e.getHash() == hash && entryKey != null &&
            ...
            if (valueReference.isLoading() ||
              ...
              // if checkTime, we want to check *after* acquiring the lock if
              // refresh still needs to be scheduled
              return null;
            }
    ...
    void enqueueNotification(@Nullable K key, int hash, @Nullable V value,
      ...
      if (map.removalNotificationQueue != DISCARDING_QUEUE) {
        RemovalNotification<K, V> notification =
            RemovalNotification.create(key, value, cause);
        map.removalNotificationQueue.offer(notification);
      }
    }

    /**
     * Performs eviction if the segment is over capacity. Avoids flushing the
     * entire cache if the newest entry exceeds the maximum weight all on its
     * own.
     *
          ...
          if (e.getHash() == hash && entryKey != null &&
            ...
            if (oldValueReference == valueReference ||
              ...
              if (oldValueReference.isActive()) {
                RemovalCause cause = (entryValue == null)
                                         ? RemovalCause.COLLECTED
                                         : RemovalCause.REPLACED;
                enqueueNotification(key, hash, entryValue,
     ...
     * during writes. If cleanup is not observed after a sufficient number of
     * reads, try cleaning up from the read thread.
     */
  ...
  static class LoadingValueReference<K, V> implements ValueReference<K, V> {
    ...
    public LoadingValueReference() { this(null); }
    ...
    public ListenableFuture<V> loadFuture(K key,
      ...
      try {
        ...
        // loadingValueReference *before* returning newValue from the cache
        // query.
        ...
        ListenableFuture<V> result =
            setException(t) ? futureValue : fullyFailedFuture(t);
   ...
   * integrated with {@code ReferenceEntry}, upon which it relies to perform its
   * linking.
   *
   * <p>Note that this entire implementation makes the assumption that all
   * elements which are in the map are also in this queue, and that all elements
   * not in the queue are not in the map.
   *
   * <p>The benefits of creating our own queue are that (1) we can replace
   * elements in the middle of the queue as part of copyWriteEntry, and (2) the
  ...
  abstract class HashIterator<T> implements Iterator<T> {
    ...
    /**
     * Advances to the given entry. Returns true if the entry was valid, false
  ...
  final class KeySet extends AbstractCacheSet<K> {
    ...
    KeySet(ConcurrentMap<?, ?> map) { super(map); }
  ...
  final class Values extends AbstractCollection<V> {
    ...
    Values(ConcurrentMap<?, ?> map) { this.map = map; }
  ...
  final class EntrySet extends AbstractCacheSet<Entry<K, V>> {
    ...
    EntrySet(ConcurrentMap<?, ?> map) { super(map); }
   ...
   * using CacheBuilder upon deserialization. An instance of this class is fit
   * for use by the writeReplace of LocalManualCache.
   *
