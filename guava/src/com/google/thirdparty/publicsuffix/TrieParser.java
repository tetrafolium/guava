...
final class TrieParser {
  ...
  parseTrie(CharSequence encoded) {
   ...
   *
   * @param stack The prefixes that precede the characters represented by this
  ...
  doParseTrieToBuilder(List<CharSequence> stack, CharSequence encoded,
    ...
    if (c == '!' || c == '?' || c == ':' || c == ',') {
      // '!' represents an interior node that represents an ICANN entry in the
      // map.
      // '?' represents a leaf node, which represents an ICANN entry in map.
      // ':' represents an interior node that represents a private entry in the
      // map
      // ',' represents a leaf node, which represents a private entry in the
      // map.
