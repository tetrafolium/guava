...
public class ExecutionListBenchmark {
  ...
  interface ExecutionListWrapper {
    ...
    void execute();
    /**
     * Returns the underlying implementation, useful for the Footprint
     * benchmark.
     */
    Object getImpl();
  }
  ...
  enum Impl {
    ...
    NEW_WITHOUT_REVERSE {
      ...
      ExecutionListWrapper newExecutionList() {
        return new ExecutionListWrapper() {
          final NewExecutionListWithoutReverse list =
              new NewExecutionListWithoutReverse();
          @Override
          public void add(Runnable runnable, Executor executor) {
            list.add(runnable, executor);
          }
    ...
    OLD_ABSTRACT_FUTURE {
      ...
      ExecutionListWrapper newExecutionList() {
        return new ExecutionListWrapper() {
          final OldAbstractFuture<Object> future =
              new OldAbstractFuture<Object>() {};
          @Override
          public void add(Runnable runnable, Executor executor) {
            future.addListener(runnable, executor);
          }
  ...
  void setUp() throws Exception {
    executorService = new ThreadPoolExecutor(
        NUM_THREADS, NUM_THREADS, Long.MAX_VALUE, TimeUnit.SECONDS,
    ...
    for (int i = 0; i < NUM_THREADS * 10; i++) {
      ...
      Future<?> possiblyIgnoredError = executorService.submit(new Runnable() {
        @Override
        public void run() {
  ...
  private static final class OldExecutionList {
    static final Logger log =
        Logger.getLogger(OldExecutionList.class.getName());
    final Queue<OldExecutionList.RunnableExecutorPair> runnables =
        Lists.newLinkedList();
  ...
  // A version of the execution list that doesn't reverse the stack in
  // execute().
  private static final class NewExecutionListWithoutReverse {
    static final Logger log =
        Logger.getLogger(NewExecutionListWithoutReverse.class.getName());

    @GuardedBy("this") private RunnableExecutorPair runnables;
    ...
    public void execute() {
      ...
      synchronized (this) {
        ...
        executed = true;
        list = runnables;
        runnables = null; // allow GC to free listeners even if this stays
                          // around for a while.
      }
      while (list != null) {
  ...
  // A version of the ExecutionList that uses an explicit tail pointer to keep
  // the nodes in order rather than flipping the stack in execute().
  private static final class NewExecutionListQueue {
    static final Logger log =
        Logger.getLogger(NewExecutionListQueue.class.getName());

    @GuardedBy("this") private RunnableExecutorPair head;
    @GuardedBy("this") private RunnableExecutorPair tail;
    @GuardedBy("this") private boolean executed;
    ...
    public void add(Runnable runnable, Executor executor) {
      ...
      synchronized (this) {
        if (!executed) {
          RunnableExecutorPair newTail =
              new RunnableExecutorPair(runnable, executor);
          if (head == null) {
            head = newTail;
            tail = newTail;
          } else {
    ...
    public void execute() {
      ...
      synchronized (this) {
        ...
        executed = true;
        list = head;
        head = null; // allow GC to free listeners even if this stays around for
                     // a while.
        tail = null;
      }
      while (list != null) {
  ...
  // A version of the list that uses compare and swap to manage the stack
  // without locks.
  private static final class ExecutionListCAS {
    static final Logger log =
        Logger.getLogger(ExecutionListCAS.class.getName());

    private static final sun.misc.Unsafe UNSAFE;
    private static final long HEAD_OFFSET;

    /**
     * A special instance of {@link RunnableExecutorPair} that is used as a
     * sentinel value for the bottom of the stack.
     */
    private static final RunnableExecutorPair NULL_PAIR =
        new RunnableExecutorPair(null, null);

    static {
      try {
        UNSAFE = getUnsafe();
        HEAD_OFFSET = UNSAFE.objectFieldOffset(
            ExecutionListCAS.class.getDeclaredField("head"));
      } catch (Exception ex) {
        throw new Error(ex);
      }
    }
    ...
    /**
     * TODO(lukes):  This was copied verbatim from Striped64.java... standardize
     * this?
     */
    ...
    public void add(Runnable runnable, Executor executor) {
      ...
      RunnableExecutorPair newHead =
          new RunnableExecutorPair(runnable, executor);
      RunnableExecutorPair oldHead;
      do {
        oldHead = head;
        if (oldHead == null) {
          // If runnables == null then execute() has been called so we should
          // just execute our listener immediately.
          newHead.execute();
          return;
        }
        ...
        newHead.next = oldHead;
          ...
          !UNSAFE.compareAndSwapObject(this, HEAD_OFFSET, oldHead, newHead));
    }
    ...
    public void execute() {
      ...
      do {
        stack = head;
        if (stack == null) {
          // If head == null then execute() has been called so we should just
          // return
          return;
        }
    ...
    private static class RunnableExecutorPair {
      final Runnable runnable;
      final Executor executor;
      // Volatile because this is written on one thread and read on another with
      // no synchronization.
