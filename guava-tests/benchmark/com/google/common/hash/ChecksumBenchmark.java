...
/**
 * Benchmarks for comparing {@link Checksum}s and {@link HashFunction}s that
 * wrap {@link Checksum}s.
 *
 * <p>Parameters for the benchmark are: <ul> <li>size: The length of the byte
 * array to hash. </ul>
 *
 * @author Colin Decker
 */
public class ChecksumBenchmark {
  ...
  @Param({"10", "1000", "100000", "1000000"}) private int size;
  ...
  private byte runHashFunction(int reps, HashFunction hashFunction) {
    byte result = 0x01;
    // Trick the JVM to prevent it from using the hash function
    // non-polymorphically
